    ...
    def _parse_npm_output(self, output: str) -> list[TestResult]:
        ...
        for line in output.splitlines():
            ...
            if match:
                ...
                status = TestResultStatus.PASS if "✓" in line or "✔" in line else TestResultStatus.FAIL
...
class ExecutionFeedbackLoop:
    ...
    def build_feedback_context(self) -> str:
        ...
        if result.all_passed:
            ...
            parts.append(f"✗ {result.failed} FAILED, {result.errors} ERRORS, {result.passed} PASSED")
        
        # Show failing tests with details
        failing = [r for r in result.results if r.status in (TestResultStatus.FAIL, TestResultStatus.ERROR)]
        if failing:
            parts.append("\nFAILING TESTS:")
