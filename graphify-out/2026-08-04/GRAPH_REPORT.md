...
## Corpus Check
- 217 files · ~152,032 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3109 nodes · 6082 edges · 180 communities (158 shown, 22 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 544 edges (avg confidence: 0.55)
...
Cohesion: 0.10
Nodes (34): test_list_dir_impl(), _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, Resolve a path relative to project root, handling ~ and absolute paths., LIST_DIR — list directory contents. (+26 more)

### Community 1 - "TieredMemory"
Cohesion: 0.07
Nodes (22): ContextSnapshot, Message, MessageRole, Tiered memory system with L0-L3 hierarchy:     - L0: Active prompt (current cont, Load persistent project memory (.context-memory.json) into L0 working state., Persist L0 working state to disk in .context-memory.json (debounced)., Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale. (+14 more)
...
Cohesion: 0.15
Nodes (11): Manage saved sessions., sessions(), CompressionConfig, VerbatimCompactor — compress text while preserving code structure., ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression. (+3 more)
...
### Community 18 - "autonomous_harness.py"
Cohesion: 0.09
Nodes (35): Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, ContentType, ExecutionMode, Enum, str, Memory models for Torchlight.  Shared data structures for message, session state, ensure_git_repository(), ensure_project_initialized() (+27 more)
...
### Community 38 - "on"
Cohesion: 0.05
Nodes (26): DirectorySelected, NamedTuple, ApprovalModal, CopySelectionModal, FileActionModal, FolderPickerModal, ModelPickerModal, on (+18 more)

### Community 39 - "core/memory/manager.py"
Cohesion: 0.07
Nodes (43): Re-export TieredMemory and MemoryConfig from shared core library core.memory.man, Adaptive, headroom-driven context budget coordinator for Torchlight.  Static res, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Flatten whitespace/newlines and truncate a scratchpad entry to a bounded length., _scratchpad_clean(), ContentChunk, ContextSnapshot, Message (+35 more)
...
### Community 41 - "ShortcutsHelpModal"
Cohesion: 0.12
...
### Community 69 - "ProjectMemory"
Cohesion: 0.20
Nodes (5): ProjectMemory, MemoryObject, SessionState, SessionPersistence, setter
...
Cohesion: 0.08
Nodes (20): AgentMemoryWidget, AgentStatusModal, ComposeResult, Displays the live L0 Agent Brain Scratchpad., Modal dialog for complete visibility into background agent actions & status tele, ComposeResult, ComposeResult, ComposeResult (+12 more)
...
Cohesion: 0.21
Nodes (14): _build_plan_text(), _make_app(), anyio, Delegate to the real TUI plan-builder helper., Repeated checklist entries (summary + detailed sections) must not duplicate., test_build_plan_text_all_done(), test_build_plan_text_dedupes_duplicate_checkbox_lines(), test_build_plan_text_goal_spec_json() (+6 more)
...
Cohesion: 0.09
Nodes (15): GoalSpec, Enum, str, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa, Return pending tasks whose dependencies are all VERIFIED., Return list of target files that collide with active or failed tasks., Construct inter-task memory prompt summarizing prior verified tasks and dependen, Run a single micro-epoch for a target task. (+7 more)
...
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `TestRunResult`, `TieredMemory`, `MemoryConfig`, `SymbolIndex`, `Console`, `on`, `ExecutionFeedbackLoop`, `cli/main.py`, `goal`, `Static`, `ShortcutsHelpModal`, `autonomous_harness.py`, `TaskSpec`, `TorchlightApp`, `tui_app.py`, `StreamingChatSession`?**
  _High betweenness centrality (0.107) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_tui_plan_panel.py`, `CloudClient`, `on`, `ShortcutsHelpModal`, `ToolCallCard`, `MessageCard`, `GitFileTree`, `test_tui_diff_view.py`, `PromptTextArea`, `OllamaClient`, `tui_app.py`, `test_tui_accessibility.py`, `AutonomousHarness`, `Step`, `LlamaCppClient`, `ConnectionPill`, `Static`, `CommandPalette`, `test_tui_theme.py`, `RLMEngineOptimized`?**
  _High betweenness centrality (0.105) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `TestRunResult`, `MemoryConfig`, `AutonomousHarness`, `ProjectMemory`, `Step`, `core/memory/manager.py`, `MemoryObject`, `cli/main.py`, `RLMEngine`, `autonomous_harness.py`, `TaskSpec`, `RLMEngineOptimized`, `test_context_budget_overflow.py`, `tool_edit_file_impl`, `StreamingChatSession`, `rlm_engine_optimized.py`, `test_plan_execution_loop.py`?**
