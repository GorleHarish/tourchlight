...
## Corpus Check
- 165 files · ~245,217 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2445 nodes · 4400 edges · 162 communities (136 shown, 26 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 202 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f04b7386`
...
- `test_rlm_engine_unwrapped_thought_parsing()` --calls--> `RLMEngine`  [EXTRACTED]
  core/tests/test_rlm_engine.py → rlm_optimized/rlm_engine.py
...
Cohesion: 0.08
Nodes (41): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_get_tool_registry(), test_tool_registry_preview_dry_run(), test_inspect_web_tool_registration(), _ddg_search(), _detect_doc_source() (+33 more)
...
Cohesion: 0.10
Nodes (17): test_rlm_engine_debate_verifier_error_resilience(), test_rlm_engine_optimized_code_execution(), test_rlm_engine_optimized_debate_verifier_initialization(), test_rlm_engine_optimized_none_tool_name(), test_rlm_engine_parse_think_tags_and_mid_sentence_prevention(), test_rlm_engine_solve_method(), test_rlm_engine_unwrapped_thought_parsing(), _clean_and_parse_json() (+9 more)
...
Cohesion: 0.15
Nodes (12): Added, Added & Improved, Added & Improved, Added & Improved, Changelog, Fixed & Improved, Fixed & Improved, [v1.7.0] - 2026-07-29 (+4 more)
...
### Community 41 - "core/memory/manager.py"
Cohesion: 0.10
Nodes (17): Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Token counting for Torchlight.  Uses tiktoken when available, falls back to a wo, _detect_apple_silicon_ram(), _detect_chip(), fetch_provider_models(), is_port_in_use(), list_available_models(), normalize_model_name() (+9 more)
...
Cohesion: 0.13
Nodes (28): _EvictingDeque, MemoryConfig, Deque that fires a callback when an item is evicted due to maxlen., Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle (+20 more)
...
### Community 45 - "ApprovalModal"
Cohesion: 0.12
Nodes (5): ApprovalModal, ModelPickerModal, Modal dialog for tool & file modification approval., Modal dialog to visually pick models and engine providers., RLMEngineOptimized
...
Cohesion: 0.10
Nodes (25): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Run fast pre-flight auto-fixer/linter on modified files before test execution. (+17 more)
...
### Community 69 - ".append"
Cohesion: 0.10
Nodes (9): _extract_dep_installs(), _extract_errors(), _extract_failing_tests(), _extract_file_paths(), MessageRole, Build a compact dev-session state summary injected at context head., MemoryNeedle, MemoryObject (+1 more)
...
### Community 80 - "test_tools_core.py"
Cohesion: 0.19
...
Cohesion: 0.07
Nodes (16): ContextSnapshot, Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale., Re-read an edited file from disk and update its pin in memory., Return list of (path, content) for pinned files., Remove all pinned files., Compress older messages, preserving the first N messages., Async wrapper for compress_recent. (+8 more)

### Community 109 - "ConversationSummarizer"
Cohesion: 0.29
...
Cohesion: 0.19
Nodes (18): AutonomousHarness, HarnessConfig, Autonomous Harness Engine driving long-running continuous execution., Ensure target project has local git repository and persistent memory initialized, main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., create_mock_feedback_loop(), ExecutionFeedbackLoop (+10 more)
...
Cohesion: 0.10
Nodes (11): ContextSnapshot, Message, Get the token breakdown bucket for a role., Add tokens to the token breakdown., Remove tokens from the token breakdown., Reset token breakdown to zero., Remove oldest non-system messages to stay under max_messages limit., Return list of (path, content) for pinned files. (+3 more)
...
### Community 151 - "ensure_project_initialized"
Cohesion: 0.70
...
## Knowledge Gaps
- **324 isolated node(s):** `Commands`, `Module Structure`, `1. 12k Context (TurboQuant Base — 12,288 Tokens)`, `2. 4k Model Fallback (4,096 Tokens)`, `Agentic Loop` (+319 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `.append`, `TokenCounter`, `LLMStateExtractor`, `context_manager/memory/manager.py`, `.__init__`, `ProjectMemory`, `build_embedder`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `cli/main.py`, `core/memory/manager.py`, `core/memory/__init__.py`, `ExecutionFeedbackLoop`, `SelectiveCompressor`, `AutonomousHarness`, `MemoryConfig`, `autonomous_harness.py`, `.__init__`, `test_context_budget_overflow.py`, `StreamingChatSession`?**
