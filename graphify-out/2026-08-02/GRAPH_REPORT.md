...
## Corpus Check
- 194 files · ~145,177 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2961 nodes · 5326 edges · 198 communities (153 shown, 45 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 300 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4d1cddd4`
...
- RLMEngineOptimized
- test_scratchpad_enhancements.py
- rules/graphify.md
...
1. `TorchlightApp` - 94 edges
2. `TieredMemory` - 83 edges
3. `TieredMemory` - 67 edges
4. `RLMEngineOptimized` - 44 edges
5. `MemoryConfig` - 43 edges
6. `AutonomousHarness` - 42 edges
7. `CommandPalette` - 31 edges
8. `ProjectSnapshot` - 31 edges
9. `tool_edit_file_impl()` - 30 edges
10. `SkillResult` - 30 edges
...
### Community 1 - "MemoryConfig"
Cohesion: 0.10
Nodes (20): ConversationSummarizer, Summarizer with LLM-powered and rule-based fallback paths.      When an llm_clie, _build_excerpt(), LLMStateExtractor, _merge_into_state(), _parse_json_response(), LLM-powered SessionState extractor.  Replaces the regex-based _merge_summary_int, Robustly extract a JSON object from the model's response.      Local models some (+12 more)
...
Cohesion: 0.12
Nodes (18): test_list_available_models_includes_gemma4e4b(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), test_normalize_mlx_gemma_4_4e4b(), _detect_apple_silicon_ram(), _detect_chip(), fetch_provider_models(), is_port_in_use() (+10 more)
...
Cohesion: 0.10
Nodes (32): ConversationSummarizer, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Extract key information from text., Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, ContentChunk, ContentType, ContextSnapshot (+24 more)
...
Cohesion: 0.10
Nodes (29): test_tool_context_window_scaling(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_file_impl() (+21 more)
...
Cohesion: 0.09
Nodes (21): ActionTracker, command, chat(), compress_file(), count_tokens(), get_phase_system_prompt(), goal(), Start an interactive chat session with context management and flashlight. (+13 more)
...
Cohesion: 0.08
Nodes (25): MemoryConfig, get_phase_system_prompt(), Unified system prompts for Torchlight.  Single source of truth for all frontends, Generate phase-tailored system prompt by appending phase instructions., Unit tests for manual context compaction trigger and 85%/91% threshold logic., test_core_compress_recent_force(), test_core_should_compress_high_ratio_low_message_count(), test_engine_compact_context() (+17 more)
...
### Community 38 - "on"
Cohesion: 0.08
Nodes (13): DirectorySelected, NamedTuple, Pressed, FileActionModal, FolderPickerModal, on, Selected, Submitted (+5 more)
...
Cohesion: 0.17
Nodes (9): Any, ReproSkill, Any, Run an async coroutine safely regardless of whether an event loop is already run, Synchronous wrapper for use from non-async contexts., Trigger real load on first call, then delegate., _run_async(), SkillResult (+1 more)

### Community 53 - "Step"
Cohesion: 0.14
Nodes (6): display_step(), get_depth_style(), Step, ApprovalModal, Modal dialog for tool & file modification approval in Cyberpunk Sci-Fi style., Snapshot file contents *before* a diffable write executes.          ``_handle_st

### Community 54 - "tui_app.py"
Cohesion: 0.27
Nodes (9): create_client(), load_last_state(), main(), _provider_runtime_info(), Torchlight Agent — Codex / Tiny-Brain 2 Style IDE TUI (Textual) Full-featured ID, Return (port, externally_managed) for a given provider key.      externally_mana, # NOTE: The model badge button click is handled by on_model_badge_clicked, save_last_state() (+1 more)
...
Cohesion: 0.08
Nodes (6): DirectoryTree, AgentStatusModal, ModelPickerModal, ComposeResult, Modal dialog to visually pick models and engine providers., Modal dialog for complete visibility into background agent actions & status tele
...
Cohesion: 0.06
Nodes (38): anyio, Binding, Changed, Tests for Phase-4 command palette + prompt autocomplete., test_build_palette_items_kinds_and_visibility(), test_command_palette_composes_filters_and_selects(), test_command_palette_enter_runs_highlighted_item(), test_fuzzy_filter_empty_query_and_no_match() (+30 more)

### Community 110 - "TieredMemory"
Cohesion: 0.07
Nodes (18): ContextSnapshot, Message, Persist L0 working state to disk in .context-memory.json., Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale., Re-read an edited file from disk and update its pin in memory., Return list of (path, content) for pinned files., Remove all pinned files. (+10 more)
...
Cohesion: 0.06
Nodes (36): test_verification_gate_allows_final_answer_when_all_done(), test_verification_gate_rejects_premature_final_answer(), test_action_tag_braces_inside_string_values(), test_action_tag_no_json_args(), test_action_tag_unclosed_with_trailing_prose(), test_clean_and_parse_json_tolerant_multiline_content(), test_clean_and_parse_json_trailing_unterminated_string(), test_repair_stop_tokens_write_file() (+28 more)

### Community 119 - "test_scratchpad_enhancements.py"
Cohesion: 0.32
Nodes (7): test_get_tool_registry(), test_tool_registry_preview_dry_run(), Unit tests for Torchlight Scratchpad and Task Harness Enhancements., test_save_memory_tool(), test_update_task_graph_tool(), get_tool_registry(), Get or create the global tool registry.
...
Cohesion: 0.14
Nodes (12): _beam_budget(), Manage saved sessions., Estimate tokens consumed by system prompt, tools, and flashlight beam., Return (max_beam_files, max_lines_per_file) for the given context size., sessions(), Ensure target project has local git repository and persistent memory initialized, test_format_l0_scratchpad_includes_pending_tasks(), Tell the tool layer what context window the current model has. (+4 more)
...
### Community 171 - "test_autonomous_harness.py"
Cohesion: 0.26
Nodes (14): HarnessConfig, main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for AutonomousHarness module., test_auto_git_init_and_clean_commit() (+6 more)
...
Cohesion: 0.17
Nodes (19): anyio, Tests for Phase-3 inline diff rendering (render_unified_diff + DiffView)., The approval modal shows a DIFF PREVIEW section when entries exist., test_approval_modal_omits_diff_when_empty(), test_approval_modal_renders_diff(), test_diff_markup_colors_lines(), test_diff_markup_truncates(), test_diff_summary_counts() (+11 more)
...
### Community 182 - "test_tui_trajectory_rail.py"
Cohesion: 0.09
Nodes (17): anyio, Tests for Phase-2 trajectory rail (pending → ok/error/denied dots)., The streamed <tool_call> adds a dot; the completing step flips it., test_app_pending_step_updates_rail(), test_rail_add_pending_and_complete_ok(), test_rail_clear_removes_dots(), test_rail_complete_error_and_denied(), test_rail_complete_without_pending_is_noop() (+9 more)
...
### Community 193 - "DiffView"
Cohesion: 0.33
Nodes (4): DiffView, ComposeResult, Container, A unified diff card: header (path + +N −M) + colored body.      Rendered client-
...
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_app_write_step_renders_diff_card`, `test_tui_plan_panel.py`, `on`, `CommandPalette`, `.open_file_tab`, `test_tui_transcript_widgets.py`, `test_tui_diff_view.py`, `GitFileTree`, `PromptTextArea`, `Step`, `test_tui_trajectory_rail.py`, `tui_app.py`, `RLMEngineOptimized`, `.action_select_mode`, `test_tui_tool_cards.py`, `AgentStatusModal`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `MemoryConfig`, `test_tui_plan_panel.py`, `on`, `TorchlightApp`, `core/memory/manager.py`, `Step`, `tui_app.py`, `.action_select_mode`, `AgentStatusModal`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `MemoryConfig`, `TokenCounter`, `_EvictingDeque`, `context_manager/memory/manager.py`, `.refresh_pin`, `ProjectMemory`, `build_embedder`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Are the 6 inferred relationships involving `TorchlightApp` (e.g. with `RLMEngineOptimized` and `Step`) actually correct?**
...
- **Are the 9 inferred relationships involving `RLMEngineOptimized` (e.g. with `AgentStatusModal` and `ApprovalModal`) actually correct?**
  _`RLMEngineOptimized` has 9 INFERRED edges - model-reasoned connections that need verification._