...
## Corpus Check
- 196 files · ~150,559 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3041 nodes · 5613 edges · 194 communities (160 shown, 34 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 325 edges (avg confidence: 0.56)
...
- .refresh_pin
- TorchlightApp
- test_session_state_execution_mode_default
- ContextSnapshot
- discovery.py
- Message
- TokenCounter
...
1. `TieredMemory` - 100 edges
2. `TorchlightApp` - 95 edges
3. `TieredMemory` - 71 edges
4. `AutonomousHarness` - 57 edges
5. `RLMEngineOptimized` - 53 edges
6. `MemoryConfig` - 49 edges
7. `ExecutionFeedbackLoop` - 36 edges
8. `StreamingChatSession` - 36 edges
9. `SkillResult` - 33 edges
10. `ProjectSnapshot` - 31 edges

## Surprising Connections (you probably didn't know these)
- `AutonomousHarness` --uses--> `TestRunResult`  [INFERRED]
  core/execution/autonomous_harness.py → context-manager-cli/src/context_manager/execution/feedback_loop.py
- `GoalSpec` --uses--> `TestRunResult`  [INFERRED]
  core/execution/autonomous_harness.py → context-manager-cli/src/context_manager/execution/feedback_loop.py
- `HarnessConfig` --uses--> `TestRunResult`  [INFERRED]
  core/execution/autonomous_harness.py → context-manager-cli/src/context_manager/execution/feedback_loop.py
- `TaskSpec` --uses--> `TestRunResult`  [INFERRED]
  core/execution/autonomous_harness.py → context-manager-cli/src/context_manager/execution/feedback_loop.py
- `TaskStatus` --uses--> `TestRunResult`  [INFERRED]
  core/execution/autonomous_harness.py → context-manager-cli/src/context_manager/execution/feedback_loop.py
...
Cohesion: 0.10
Nodes (25): test_list_dir_impl(), _auto_repair(), _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, Resolve a path relative to project root, handling ~ and absolute paths. (+17 more)
...
Cohesion: 0.17
Nodes (24): ContentChunk, ContentType, ContextSnapshot, ExecutionMode, MemoryNeedle, MemoryObject, Message, MessageRole (+16 more)
...
Cohesion: 0.11
Nodes (27): test_verify_compile_param(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_file_impl() (+19 more)

### Community 14 - "WebOutcomeInspector"
Cohesion: 0.13
Nodes (14): EphemeralHTTPServer, Any, Path, QuietHTTPRequestHandler, Web Outcome Inspector for Torchlight.  Provides low-memory, ephemeral runtime an, Spins up a lightweight local HTTP server for static file inspection., Main Inspector Subsystem driving zero-memory, ephemeral web verification., Tier 3: Run Node JSDOM script if node is available. (+6 more)
...
Cohesion: 0.09
Nodes (5): ComposeResult, AgentStatusModal, Modal dialog for selecting session execution mode (Chat vs Goal)., Modal dialog for complete visibility into background agent actions & status tele, SessionModePickerModal
...
### Community 34 - "rlm_engine.py"
Cohesion: 0.17
Nodes (10): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), build_step_message() (+2 more)
...
### Community 38 - "on"
Cohesion: 0.12
Nodes (8): DirectorySelected, on, FileActionModal, FolderPickerModal, Modal dialog for interactive visual folder selection across the entire computer., Modal dialog presenting OS options when a file is selected in the Explorer tree., Selected, Submitted
...
Cohesion: 0.10
Nodes (33): MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject, Message (+25 more)
...
Cohesion: 0.12
Nodes (23): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_read_symbols_indented_methods_and_duplicate_names(), test_run_command_intercept_ast_functions(), test_search_ast_action_aliases(), test_search_ast_impl_fallback(), test_read_symbols_impl(), READ_SYMBOLS — show file structure without loading content., SAVE_MEMORY — save a fact, decision, or failed strategy to project memory. (+15 more)
...
Cohesion: 0.11
Nodes (11): _clamp(), ContextBudget, Adaptive, headroom-driven context budget coordinator for Torchlight.  Static res, Token reserve kept for the recent-message window., Current fraction of the target window in use., Effective budget allocations for the current turn.      `used_tokens` is the liv, Token allowance for the L0 working memory scratchpad this turn., Max characters per scratchpad entry (longer when headroom is ample). (+3 more)
...
### Community 55 - "context_manager/memory/manager.py"
Cohesion: 0.21
...
### Community 60 - "LLMStateExtractor"
Cohesion: 0.13
...
Cohesion: 0.07
Nodes (35): test_classify_confirm_commands(), test_classify_destructive_commands(), test_classify_empty_command(), test_classify_safe_commands(), test_classify_unknown_defaults_to_confirm(), test_classify_whitespace_handling(), anyio, Tests for Phase-2 tool call cards (risk badge, status, timing, sections). (+27 more)
...
Cohesion: 0.13
Nodes (27): Unit tests for Torchlight Zero-Context Code Quality Harness., test_check_syntax_js_bracket_balance(), test_check_syntax_js_string_literal_brackets(), test_check_syntax_json(), test_check_syntax_python(), test_compile_gate_rejects_return_outside_function(), test_force_bypasses_validation_gates(), test_format_code_on_save_fallback() (+19 more)
...
Cohesion: 0.40
Nodes (4): get_phase_system_prompt(), Unified system prompts for Torchlight.  Single source of truth for all frontends, Generate phase-tailored system prompt by appending phase instructions., test_phase_system_prompt_generation()
...
Cohesion: 0.14
Nodes (9): Changed, Highlighted, NamedTuple, CommandPalette, PaletteResult, on, Selected, Submitted (+1 more)

### Community 110 - "Message"
Cohesion: 0.13
Nodes (7): Persist L0 working state to disk in .context-memory.json., Remove all pinned files., Compress older messages, preserving the first N messages., Async wrapper for compress_recent., Compact context between tasks while preserving continuous session state., Record an explicit memory entry into SessionState and persist to project memory., Message
...
### Community 113 - "test_autonomous_harness.py"
Cohesion: 0.18
Nodes (23): HarnessConfig, TestResult, create_mock_feedback_loop(), create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for Inter-Task Context Pipeline, Dependencies, and File Collision Gua, test_inter_task_output_summary_injection() (+15 more)
...
Cohesion: 0.08
Nodes (25): test_action_tag_braces_inside_string_values(), test_action_tag_no_json_args(), test_action_tag_unclosed_with_trailing_prose(), test_clean_and_parse_json_tolerant_multiline_content(), test_clean_and_parse_json_trailing_unterminated_string(), test_repair_stop_tokens_write_file(), test_rlm_engine_code_tag_and_backticks(), test_rlm_engine_debate_verifier_error_resilience() (+17 more)

### Community 119 - "test_tools_core.py"
Cohesion: 0.16
...
Cohesion: 0.14
Nodes (18): anyio, _build_plan_text(), _make_app(), Delegate to the real TUI plan-builder helper., Repeated checklist entries (summary + detailed sections) must not duplicate., test_build_plan_text_all_done(), test_build_plan_text_dedupes_duplicate_checkbox_lines(), test_build_plan_text_goal_spec_json() (+10 more)
...
Cohesion: 0.14
Nodes (12): HTMLParser, Tier 1: Static HTML syntax and asset path validator., StaticHTMLValidator, test_search_ast_schema_validation(), test_get_tool_registry(), test_tool_registry_preview_dry_run(), Tests for WebOutcomeInspector and INSPECT_WEB tool., test_inspect_web_tool_registration() (+4 more)

### Community 149 - "AutonomousHarness"
Cohesion: 0.10
Nodes (20): AutonomousHarness, GoalSpec, Enum, ExecutionFeedbackLoop, Path, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Ensure target project has local git repository and persistent memory initialized, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa (+12 more)

### Community 150 - ".format_l0_scratchpad"
Cohesion: 0.13
...
### Community 155 - "tool_write_file_impl"
Cohesion: 0.15
Nodes (11): test_detect_stubs(), test_tool_write_file_integration(), test_write_file_blocks_broken_syntax_and_truncation(), test_write_file_impl(), test_write_file_impl_missing_path(), Tests for performance and accuracy optimizations in Torchlight., test_inline_syntax_guardrail(), _detect_stubs() (+3 more)
...
### Community 158 - "rlm_engine_optimized.py"
Cohesion: 0.28
Nodes (5): Return True if the most recent test run actually ran and has failing or, Build an explicit warning attached to an accepted final answer when the, SolveResult, Validate tool call against schema and normalize parameter aliases.      Returns:, validate_and_normalize_tool_call()

### Community 159 - "ModelPickerModal"
Cohesion: 0.22
...
### Community 162 - "CopySelectionModal"
Cohesion: 0.22
...
Cohesion: 0.06
Nodes (39): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Freshly verify any modified-but-unverified files and return True if         ever (+31 more)

### Community 171 - "TorchlightApp"
Cohesion: 0.07
Nodes (9): App, FileSelected, NodeSelected, Codex / Tiny-Brain 2 Style Agent IDE TUI., Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline., Mount a running ToolCallCard for a streamed ``<tool_call>`` marker.          Kep, TorchlightApp (+1 more)
...
### Community 197 - "._handle_slash_command"
Cohesion: 0.10
Nodes (7): Mount a widget defensively and scroll safely after layout pass., Build the AST knowledge graph silently in background thread., Consolidated Phase-4 status bar (state · model · context gauge · tps · tokens ·, Repoint the file tree at the engine root and refresh git status., Manually trigger memory context compaction., StreamingView, work
...
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `.__init__`, `ModelPickerModal`, `CopySelectionModal`, `test_tui_plan_panel.py`, `on`, `._handle_slash_command`, `cli/main.py`, `TieredMemory`, `TorchlightApp`, `ExecutionFeedbackLoop`, `test_autonomous_harness.py`, `ApprovalModal`, `tui_app.py`, `StreamingChatSession`, `AgentStatusModal`?**
  _High betweenness centrality (0.080) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `.__init__`, `RLMEngine`, `TokenCounter`, `cli/main.py`, `_EvictingDeque`, `.refresh_pin`, `MemoryConfig`, `ProjectMemory`, `context_manager/memory/manager.py`, `build_embedder`, `StreamingChatSession`, `LLMStateExtractor`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `rlm_engine.py`, `test_context_budget_overflow.py`, `RLMEngine`, `cli/main.py`, `core/memory/models.py`, `test_plan_execution_loop.py`, `Message`, `test_autonomous_harness.py`, `ContextBudget`, `AutonomousHarness`, `.format_l0_scratchpad`, `RLMEngineOptimized`, `tool_edit_file_impl`, `Step`, `rlm_engine_optimized.py`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Are the 9 inferred relationships involving `TieredMemory` (e.g. with `AutonomousHarness` and `GoalSpec`) actually correct?**
...
- **Are the 13 inferred relationships involving `AutonomousHarness` (e.g. with `StreamingChatSession` and `ExecutionFeedbackLoop`) actually correct?**
  _`AutonomousHarness` has 13 INFERRED edges - model-reasoned connections that need verification._