...
## Corpus Check
- 176 files · ~129,385 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2589 nodes · 4709 edges · 180 communities (145 shown, 35 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 253 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5f831739`
...
3. `TorchlightApp` - 66 edges
4. `MemoryConfig` - 45 edges
5. `AutonomousHarness` - 42 edges
6. `RLMEngineOptimized` - 40 edges
...
Cohesion: 0.10
Nodes (35): _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, LIST_DIR — list directory contents., WEB_SEARCH — general web search., DOC_SEARCH — search official documentation. (+27 more)
...
Cohesion: 0.05
Nodes (41): test_tool_registry_execute(), test_tool_registry_execute_unknown(), test_tool_registry_get(), test_tool_registry_register(), test_tool_registry_risk_level(), test_tool_registry_risk_level_run_command(), test_tool_result_failure(), test_tool_result_success() (+33 more)
...
Cohesion: 0.15
Nodes (23): ContentChunk, ContentType, ContextSnapshot, ExecutionMode, MemoryNeedle, MemoryObject, Message, MessageRole (+15 more)
...
Cohesion: 0.13
Nodes (22): test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_list_dir_impl(), test_read_file_impl() (+14 more)
...
Cohesion: 0.10
Nodes (28): ensure_git_repository(), ensure_project_initialized(), init_new_project(), ProjectMemory, Path, SessionState, Session and project memory persistence for Torchlight., Ensure target project directory exists and has `.context-memory.json` persistent (+20 more)
...
Cohesion: 0.05
Nodes (37): test_list_available_models_includes_gemma4e4b(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), test_normalize_mlx_gemma_4_4e4b(), test_rlm_engine_solve_method(), index_directory(), IndexVisitor, init_db() (+29 more)

### Community 27 - "StreamingChatSession"
Cohesion: 0.17
Nodes (7): chat(), Start an interactive chat session with context management and flashlight., Estimate tokens consumed by system prompt, tools, and flashlight beam., Run out-of-band DebateVerifier pass if candidate proposal needs verification., /params                    — show current params         /params auto, StreamingChatSession, Panel
...
Cohesion: 0.09
Nodes (11): App, copy_to_clipboard(), Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline., Mount a widget defensively and scroll safely after layout pass., Build the AST knowledge graph for the current project_root in a         backgrou, Manually trigger memory context compaction., Copy text to system clipboard across macOS, Linux, and Windows. (+3 more)
...
### Community 46 - "core/memory/manager.py"
Cohesion: 0.19
Nodes (11): Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, get_token_counter(), Token counting for Torchlight.  Uses tiktoken when available, falls back to a wo, TokenCounter, test_get_token_counter_caching(), test_get_token_counter_different_models(), test_token_counter_basic(), test_token_counter_empty() (+3 more)
...
### Community 53 - "verify_m1_setup.py"
Cohesion: 0.18
...
Cohesion: 0.07
Nodes (16): ContextSnapshot, Message, Persist L0 working state to disk in .context-memory.json., Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale., Re-read an edited file from disk and update its pin in memory., Return list of (path, content) for pinned files., Remove all pinned files. (+8 more)
...
Cohesion: 0.13
Nodes (12): ExecutionFeedbackLoop, Path, Auto-run tests and web outcome inspection after code changes and inject feedback, Return True if the most recent test run has failing or error tests., Called after a tool is executed. Returns test results if tests were run., Run fast pre-flight auto-fixer/linter on modified files before test execution., Detect and run the project's test suite or web inspector., Convert current failing TestRunResult into a structured TestFailureError for Rec (+4 more)
...
Cohesion: 0.09
Nodes (21): anyio, test_format_l0_scratchpad_includes_pending_tasks(), test_verification_gate_allows_final_answer_when_all_done(), test_verification_gate_rejects_premature_final_answer(), test_rlm_engine_code_tag_and_backticks(), test_rlm_engine_debate_verifier_error_resilience(), test_rlm_engine_optimized_code_execution(), test_rlm_engine_optimized_debate_verifier_initialization() (+13 more)
...
### Community 125 - "core/compression/summarizer.py"
Cohesion: 0.24
...
Cohesion: 0.15
Nodes (12): _build_plan_text_isolated(), Isolated plan builder matching TorchlightApp logic., test_build_plan_text_all_done(), test_build_plan_text_goal_spec_json(), test_build_plan_text_no_file(), test_build_plan_text_with_tasks(), test_copy_selection_modal_composes(), test_shortcuts_help_modal_composes() (+4 more)
...
Cohesion: 0.17
Nodes (11): AutonomousHarness, GoalSpec, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa, Return pending tasks whose dependencies are all VERIFIED., Return list of target files that collide with active or failed tasks., Construct inter-task memory prompt summarizing prior verified tasks and dependen, Run a single micro-epoch for a target task., Run continuous autonomous daemon until completion or timeout. (+3 more)
...
Cohesion: 0.38
Nodes (4): Ensure target project has local git repository and persistent memory initialized, ExecutionFeedbackLoop, Path, TieredMemory
...
### Community 173 - "SessionModePickerModal"
Cohesion: 0.22
Nodes (3): Modal dialog for selecting session execution mode (Chat vs Goal)., SessionModePickerModal, setter
...
Cohesion: 0.43
Nodes (4): Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, TaskStatus, Enum, str

## Knowledge Gaps
- **348 isolated node(s):** `Summary of Tracked Bugs`, `BUG-01: `_total_llm_calls` Counter Accumulation Across Sessions`, `BUG-02: Verification Gate Premature Answer Bypass via Rejection State`, `BUG-03: Autonomous Harness Daemon Infinite Loop on Exception`, `BUG-04: Consecutive Code Error Loop Unbounded Retries` (+343 more)
...
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `BaseSkill`, `HTMLGameSkill`, `TDDSkill`, `MarkdownDocumentSkill`, `CalculatorSkill`, `PlanningSkill`, `discovery.py`, `TDDSkill`?**
  _High betweenness centrality (0.037) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_tui_plan_panel.py`, `on`, `Step`, `SessionModePickerModal`, `.on_tab_action_button_pressed`, `Static`, `tui_app.py`, `RLMEngineOptimized`, `ApprovalModal`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `MemoryConfig`, `test_tui_plan_panel.py`, `on`, `Step`, `TorchlightApp`, `SessionModePickerModal`, `core/memory/manager.py`, `Static`, `tui_app.py`, `ApprovalModal`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
...
- **Are the 2 inferred relationships involving `TorchlightApp` (e.g. with `RLMEngineOptimized` and `Step`) actually correct?**
  _`TorchlightApp` has 2 INFERRED edges - model-reasoned connections that need verification._
