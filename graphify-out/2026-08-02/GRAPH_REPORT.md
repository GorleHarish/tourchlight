...
## Corpus Check
- 199 files · ~151,359 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3050 nodes · 5947 edges · 175 communities (152 shown, 23 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 511 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1b25193e`
...
2. `TieredMemory` - 96 edges
3. `TieredMemory` - 74 edges
4. `RLMEngineOptimized` - 60 edges
5. `AutonomousHarness` - 59 edges
6. `MemoryConfig` - 52 edges
7. `ExecutionFeedbackLoop` - 45 edges
8. `LlamaCppClient` - 42 edges
...
- `CopySelectionModal` --uses--> `AutonomousHarness`  [INFERRED]
  rlm_optimized/tui_app.py → core/execution/autonomous_harness.py
- `FileActionModal` --uses--> `AutonomousHarness`  [INFERRED]
  rlm_optimized/tui_app.py → core/execution/autonomous_harness.py
...
Cohesion: 0.11
Nodes (24): _auto_repair(), _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, Resolve a path relative to project root, handling ~ and absolute paths., LIST_DIR — list directory contents. (+16 more)

### Community 1 - "GitFileTree"
Cohesion: 0.22
Nodes (9): DirectoryTree, DirEntry, GitFileTree, Path, Filter out OS noise, cache directories, and internal state files., DirectoryTree whose file labels carry git status decorations., Filter out hidden dotfiles and OS clutter from tree view., Prefix file labels with clean git status badges ([U] untracked, [M] modified, et (+1 more)
...
Cohesion: 0.06
Nodes (39): test_tool_registry_execute(), test_tool_registry_execute_unknown(), test_tool_registry_get(), test_tool_registry_register(), test_tool_registry_risk_level(), test_tool_registry_risk_level_run_command(), test_tool_result_failure(), test_tool_result_success() (+31 more)
...
### Community 10 - "core/tests/test_models.py"
Cohesion: 0.15
Nodes (18): ContentChunk, ContextSnapshot, MemoryNeedle, MemoryObject, Snapshot of current memory state for display., SessionState, WorkingSetSnapshot, test_content_chunk() (+10 more)
...
Cohesion: 0.12
Nodes (23): test_verify_compile_param(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_list_dir_impl() (+15 more)
...
Cohesion: 0.09
Nodes (27): ensure_git_repository(), ensure_project_initialized(), init_new_project(), ProjectMemory, Path, SessionState, Session and project memory persistence for Torchlight., Ensure target project directory exists and has `.context-memory.json` persistent (+19 more)
...
Cohesion: 0.13
Nodes (12): command, chat(), goal(), Panel, Start an interactive chat session with context management and flashlight., Start an autonomous goal execution session driven by .torchlight task tracking., Infer the current agent phase from user input and the last model response., Auto-switch _params based on detected phase.  No-op when locked. (+4 more)
...
Cohesion: 0.07
Nodes (25): Horizontal, Pressed, AgentStatusModal, CopySelectionModal, ModelPickerModal, ComposeResult, Modal dialog to visually pick models and engine providers., Modal dialog to select and copy specific messages, code blocks, or text turns. (+17 more)
...
### Community 33 - "RLMEngine"
Cohesion: 0.09
Nodes (28): ConversationSummarizer, DevSessionSummarizer, _extract_code_signatures(), _extract_errors(), _extract_failing_tests(), _extract_file_paths(), _format_messages_for_summary(), IncrementalSummarizer (+20 more)

### Community 34 - "test_prompts_and_memory.py"
Cohesion: 0.18
...
Cohesion: 0.10
Nodes (29): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_read_symbols_indented_methods_and_duplicate_names(), test_run_command_intercept_ast_functions(), test_search_ast_action_aliases(), test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_read_symbols_impl(), test_run_command_impl() (+21 more)

### Community 46 - "MessageCard"
Cohesion: 0.08
Nodes (25): anyio, Tests for Phase-1 transcript widgets (message cards, streaming, thinking)., Smoke test: the real app mounts MessageCards and drives the streaming view., test_app_transcript_wiring(), test_card_meta_for(), test_estimate_token_count(), test_message_card_composes(), test_streaming_view_updates() (+17 more)
...
Cohesion: 0.19
Nodes (13): Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, CompressionLevel, Enum, Selective Memory Compression — Progressive context reduction for local LLMs.  4-, get_token_counter(), Token counting for Torchlight.  Uses tiktoken when available, falls back to a wo, TokenCounter, test_get_token_counter_caching() (+5 more)

### Community 50 - "tool_read_file"
Cohesion: 0.16
...
Cohesion: 0.08
Nodes (13): _clamp(), ContextBudget, Adaptive, headroom-driven context budget coordinator for Torchlight.  Static res, Token reserve kept for the recent-message window., Current fraction of the target window in use., Effective budget allocations for the current turn.      `used_tokens` is the liv, Token allowance for the L0 working memory scratchpad this turn., Max characters per scratchpad entry (longer when headroom is ample). (+5 more)

### Community 54 - "tui_app.py"
Cohesion: 0.09
Nodes (27): test_list_available_models_includes_gemma4e4b(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), test_normalize_mlx_gemma_4_4e4b(), fetch_provider_models(), list_available_models(), normalize_model_name(), Normalize model alias names (e.g. 'gemma-2-2b', 'qwen', 'gemma 4 E2B', 'gemma 4 (+19 more)

### Community 55 - "tool_write_file_impl"
Cohesion: 0.15
Nodes (13): test_detect_stubs(), test_tool_write_file_integration(), test_write_file_blocks_broken_syntax_and_truncation(), test_write_file_impl(), test_write_file_impl_missing_path(), Tests for performance and accuracy optimizations in Torchlight., test_batch_tool_execution(), test_inline_syntax_guardrail() (+5 more)
...
### Community 61 - "ToolCallCard"
Cohesion: 0.07
Nodes (36): test_classify_confirm_commands(), test_classify_destructive_commands(), test_classify_empty_command(), test_classify_safe_commands(), test_classify_unknown_defaults_to_confirm(), test_classify_whitespace_handling(), anyio, Tests for Phase-2 tool call cards (risk badge, status, timing, sections). (+28 more)

### Community 62 - "command_palette.py"
Cohesion: 0.18
Nodes (10): test_match_prompt_suggestions_file_at(), test_match_prompt_suggestions_slash(), test_slash_command_list_shape(), _fuzzy_score(), match_prompt_suggestions(), Command palette + slash-command autocomplete for the Torchlight TUI.  Phase 4: *, Suggest completions for a single-token ``/cmd`` or ``@file`` prefix., (command, usage, description) triples mirrored from ``_handle_slash_command``. (+2 more)
...
Cohesion: 0.13
Nodes (11): TokenCounter, Load persistent project memory (.context-memory.json) into L0 working state., CompressionConfig, Pattern, Progressive compression that preserves semantic meaning.      Strategy:     - Re, Compress a list of messages using progressive levels., SelectiveCompressor, TurnSummary (+3 more)

### Community 66 - "InferenceParams"
Cohesion: 0.17
Nodes (7): InferenceParams, Emitting <plan> blocks and <thought> reasoning.         Some creativity for step, Analysing errors and diagnosing failures.         Moderate exploration to surfac, General conversation and clarification — default settings., One-line human-readable summary for the dashboard., Sampling parameters forwarded to the LM Studio /chat/completions endpoint.     O, Writing code files.  Near-deterministic — exact syntax matters.
...
### Community 69 - "test_autonomous_harness.py"
Cohesion: 0.18
Nodes (19): HarnessConfig, Return True only if a run succeeded. Uses exit code as the authoritative, TestResult, TestRunResult, create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for AutonomousHarness module. (+11 more)

### Community 70 - "test_code_quality_harness.py"
Cohesion: 0.12
Nodes (29): Unit tests for Torchlight Zero-Context Code Quality Harness., test_check_syntax_js_bracket_balance(), test_check_syntax_js_string_literal_brackets(), test_check_syntax_json(), test_check_syntax_python(), test_compile_gate_rejects_return_outside_function(), test_edit_file_blocks_broken_syntax(), test_force_bypasses_validation_gates() (+21 more)
...
Cohesion: 0.10
Nodes (15): anyio, Tests for Phase-2 trajectory rail (pending → ok/error/denied dots)., The streamed <tool_call> adds a dot; the completing step flips it., test_app_pending_step_updates_rail(), test_rail_add_pending_and_complete_ok(), test_rail_clear_removes_dots(), test_rail_complete_error_and_denied(), test_rail_complete_without_pending_is_noop() (+7 more)
...
### Community 104 - "cli/main.py"
Cohesion: 0.14
Nodes (11): _beam_budget(), compress_file(), get_phase_system_prompt(), Compress a file using verbatim compaction., Estimate tokens consumed by system prompt, tools, and flashlight beam., Return (max_beam_files, max_lines_per_file) for the given context size., _risk_tier(), _tool_kind() (+3 more)
...
### Community 113 - "rlm_engine_optimized.py"
Cohesion: 0.10
Nodes (24): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), ContentType (+16 more)
...
Cohesion: 0.11
Nodes (28): test_action_tag_braces_inside_string_values(), test_action_tag_no_json_args(), test_action_tag_unclosed_with_trailing_prose(), test_clean_and_parse_json_tolerant_multiline_content(), test_clean_and_parse_json_trailing_unterminated_string(), test_repair_stop_tokens_write_file(), test_rlm_engine_code_tag_and_backticks(), test_rlm_engine_debate_verifier_error_resilience() (+20 more)
...
### Community 125 - "test_autonomous_harness_pipeline.py"
Cohesion: 0.43
...
Cohesion: 0.10
Nodes (10): _extract_errors(), ContextSnapshot, Message, MessageRole, Return list of (path, content) for pinned files., Build context using selective progressive compression.          Kept as a standa, Build a compact dev-session state summary injected at context head., TieredMemory (+2 more)
...
Cohesion: 0.09
Nodes (23): AutonomousHarness, GoalSpec, Enum, Path, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Ensure target project has local git repository and persistent memory initialized, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa, Return pending tasks whose dependencies are all VERIFIED. (+15 more)
...
Cohesion: 0.06
Nodes (37): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Freshly verify any modified-but-unverified files and return True if         ever (+29 more)
...
Cohesion: 0.26
Nodes (11): anyio, Tests for Phase-4 command palette + prompt autocomplete., test_command_palette_composes_filters_and_selects(), test_command_palette_enter_runs_highlighted_item(), test_fuzzy_filter_empty_query_and_no_match(), test_fuzzy_filter_prefix_beats_substring(), test_prompt_text_area_enter_submits_and_accepts_suggestion(), test_prompt_text_area_file_suggestions_and_dismiss() (+3 more)
...
Cohesion: 0.05
Nodes (19): App, is_port_in_use(), Check if server port 8080 is actively listening., copy_to_clipboard(), Step, Codex / Tiny-Brain 2 Style Agent IDE TUI., Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline. (+11 more)

## Knowledge Gaps
- **363 isolated node(s):** `Commands`, `Module Structure`, `1. 12k Context (TurboQuant Base — 12,288 Tokens)`, `2. 4k Model Fallback (4,096 Tokens)`, `Agentic Loop` (+358 more)
...
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `test_autonomous_harness.py`, `MemoryConfig`, `on`, `cli/main.py`, `ExecutionFeedbackLoop`, `ShortcutsHelpModal`, `TorchlightApp`, `tui_app.py`, `StreamingChatSession`, `test_autonomous_harness_pipeline.py`, `AgentStatusModal`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `RLMEngine`, `TokenCounter`, `cli/main.py`, `_EvictingDeque`, `context_manager/memory/manager.py`, `LlamaCppClient`, `ProjectMemory`, `RLMEngineOptimized`, `build_embedder`, `StreamingChatSession`, `.clear`?**
  _High betweenness centrality (0.056) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `GitFileTree`, `ShortcutsHelpModal`, `on`, `CloudClient`, `CommandPalette`, `MessageCard`, `test_tui_diff_view.py`, `LlamaCppClient`, `PromptTextArea`, `AutonomousHarness`, `tui_app.py`, `OllamaClient`, `RLMEngineOptimized`, `TrajectoryRail`, `ToolCallCard`, `AgentStatusModal`?**
...
- **Are the 20 inferred relationships involving `TieredMemory` (e.g. with `sessions()` and `StreamingChatSession`) actually correct?**
