# Graph Report - tourchlight v1_i6  (2026-08-09)

## Corpus Check
- 225 files · ~166,949 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3369 nodes · 6856 edges · 187 communities (166 shown, 21 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 620 edges (avg confidence: 0.55)
...
1. `TorchlightApp` - 161 edges
2. `TieredMemory` - 144 edges
3. `RLMEngineOptimized` - 90 edges
4. `MemoryConfig` - 81 edges
5. `AutonomousHarness` - 62 edges
6. `ExecutionFeedbackLoop` - 53 edges
7. `LlamaCppClient` - 44 edges
8. `StreamingChatSession` - 41 edges
9. `CloudClient` - 38 edges
10. `Step` - 37 edges
...
Cohesion: 0.13
Nodes (21): calculate_in_memory_diff(), Calculate exact lines added and deleted between two string buffers in RAM., test_detect_stubs(), test_calculate_in_memory_diff(), _auto_repair(), _commit_edit_and_format_result(), _commit_edit_file(), _ddg_search() (+13 more)
...
Cohesion: 0.10
Nodes (28): test_verify_compile_param(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_list_dir_impl() (+20 more)

### Community 14 - "HtmlGamePlayer"
Cohesion: 0.10
Nodes (31): GameInputEvent, GameOutcomeResult, get_process_memory_mb(), HtmlGamePlayer, Path, HTML Game Inspector & Player Harness for Torchlight.  Provides autonomous playin, Autonomous HTML Game Harness & Dynamic Verifier.      Launches an ephemeral brow, Plays the HTML game for duration_ms while simulating inputs and checking frame d (+23 more)

### Community 15 - "registry.py"
Cohesion: 0.09
Nodes (28): _git_run(), play_and_verify_game(), Any, WEB_SEARCH — general web search., SAVE_MEMORY — save a fact, decision, or failed strategy to project memory., FORMAT_CODE — beautify a code snippet., ASK_USER — ask the user a question., SET_PHASE — switch active agent phase. (+20 more)
...
### Community 19 - "validate_tool_call"
Cohesion: 0.11
...
### Community 21 - "test_tui_tool_cards.py"
Cohesion: 0.15
...
Cohesion: 0.18
Nodes (8): goal(), Panel, /params                    — show current params         /params auto, Start an autonomous goal execution session driven by .torchlight task tracking., Run out-of-band DebateVerifier pass if candidate proposal needs verification., Step function for AutonomousHarness - executes a single task iteration., Async implementation of harness step function., StreamingChatSession

### Community 28 - "._execute_tool_with_approval"
Cohesion: 0.25
...
Cohesion: 0.09
Nodes (27): Verify Goal mode detects missing implementation_plan.md and forces 'plan' phase., Verify bare JSON tool calls without <tool_call> tags are correctly parsed as too, test_detect_phase_goal_mode_missing_plan_forces_plan(), test_parse_response_bare_json_tool_call(), test_action_tag_braces_inside_string_values(), test_action_tag_no_json_args(), test_action_tag_unclosed_with_trailing_prose(), test_inline_interception_requires_explicit_file_or_header() (+19 more)
...
### Community 38 - "tui_app.py"
Cohesion: 0.10
Nodes (26): test_list_available_models_includes_gemma4e4b(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), test_normalize_mlx_gemma_4_4e4b(), _detect_apple_silicon_ram(), _detect_chip(), fetch_provider_models(), list_available_models() (+18 more)

### Community 39 - "rlm_engine_optimized.py"
Cohesion: 0.12
Nodes (34): Re-export TieredMemory and MemoryConfig from shared core library core.memory.man, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Flatten whitespace/newlines and truncate a scratchpad entry to a bounded length., _scratchpad_clean(), ContentChunk, ContentType, ContextSnapshot (+26 more)
...
### Community 47 - "test_prompts_and_memory.py"
Cohesion: 0.08
Nodes (27): extract_modified_symbols(), is_valid_file_path(), Explicitly record a modified file, Net Delta line stats, and touched symbols in, Explicitly record a read file in session state if it is a valid file path., Validate if a string is a genuine file path rather than code attribute access (e, Extract function/class AST symbol names modified or added between old and new te, Persist L0 working state to disk in .context-memory.json (debounced)., get_phase_system_prompt() (+19 more)
...
Cohesion: 0.08
Nodes (24): _is_valid_decision(), Filter out empty, generic, or noisy session summary strings., Load persistent project memory (.context-memory.json) into L0 working state., CompressionConfig, CompressionLevel, Enum, Pattern, Selective Memory Compression — Progressive context reduction for local LLMs.  4- (+16 more)
...
### Community 53 - "WebOutcomeInspector"
Cohesion: 0.09
Nodes (19): Any, HTMLParser, Path, QuietHTTPRequestHandler, Web Outcome Inspector for Torchlight.  Provides low-memory, ephemeral runtime an, Tier 1: Static HTML syntax and asset path validator., Main Inspector Subsystem driving zero-memory, ephemeral web verification., Tier 3: Run Node JSDOM script if node is available. (+11 more)

### Community 54 - "sync_workspace_tasks"
Cohesion: 0.09
Nodes (35): test_add_subtask_survives_sync_and_lands_in_plan(), test_insert_task_into_plan_section(), test_update_task_graph_syncs_plan(), UPDATE_TASK_GRAPH — dynamically mutate sub-tasks in .torchlight/goal_spec.json., tool_update_task_graph_impl(), _clean_task_text(), _file_looks_complete(), insert_task_into_plan() (+27 more)

### Community 55 - "UnifiedSkillRegistry"
Cohesion: 0.21
...
### Community 69 - "tool_write_file_impl"
Cohesion: 0.15
...
Cohesion: 0.04
Nodes (27): is_port_in_use(), Check if server port 8080 is actively listening., copy_to_clipboard(), App, Step, Copy text to system clipboard across macOS, Linux, and Windows., Codex / Tiny-Brain 2 Style Agent IDE TUI., Realtime task-state updates surfaced to the Output log. (+19 more)

### Community 83 - "setup_optimized.sh"
Cohesion: 0.60
Nodes (3): info(), ok(), setup_optimized.sh script

### Community 84 - "CenterEmptyState"
Cohesion: 0.18
...
### Community 105 - "classify_command"
Cohesion: 0.26
...
Cohesion: 0.07
Nodes (21): AgentMemoryWidget, AgentStatusModal, ComposeResult, Displays the live L0 Agent Brain Scratchpad., Modal dialog for complete visibility into background agent actions & status tele, ComposeResult, ComposeResult, ComposeResult (+13 more)
...
### Community 139 - "test_ast_tool.py"
Cohesion: 0.21
...
### Community 157 - "CommandPalette"
Cohesion: 0.05
Nodes (39): Binding, Changed, anyio, Tests for Phase-4 command palette + prompt autocomplete., test_build_palette_items_kinds_and_visibility(), test_command_palette_composes_filters_and_selects(), test_command_palette_enter_runs_highlighted_item(), test_fuzzy_filter_empty_query_and_no_match() (+31 more)

### Community 158 - "LlamaCppClient"
Cohesion: 0.06
Nodes (25): CloudClient, Sanitize message roles. Convert system role to user role for models (e.g. Gemma, Async implementation of chat protocol method required by LLMClient and DebateVer, Async streaming implementation required by LLMClient protocol., Return the ids of models the provider currently reports as available.         Us, Resolve requested self.model against live models to prevent 404 mismatches., _sanitize_messages_for_cloud(), LlamaCppClient (+17 more)
...
### Community 162 - "get_tool_registry"
Cohesion: 0.33
Nodes (6): test_get_tool_registry(), test_tool_registry_preview_dry_run(), test_registry_get_description_block_phase(), test_save_memory_tool(), get_tool_registry(), Get or create the global tool registry.
...
### Community 167 - "chat"
Cohesion: 0.40
Nodes (5): command, chat(), compress_file(), Start an interactive chat session with context management and flashlight., Compress a file using verbatim compaction.
...
### Community 178 - "core.py"
Cohesion: 0.13
Nodes (19): Core Tools — built-in, always available, called via TOOL_NAME("arg") syntax.  Th, Unit tests for context budget overflow detection and fixes in TieredMemory, RLME, test_tiered_memory_total_tokens_includes_pinned_files(), test_tool_context_window_scaling(), test_read_file_impl(), test_read_file_impl_not_found(), _extract_symbols(), Return [(lineno_1based, kind, name), ...] sorted by line number. (+11 more)

### Community 179 - "TestApp"
Cohesion: 0.22
Nodes (5): App, ComposeResult, on, Pressed, TestApp

### Community 180 - "cli/main.py"
Cohesion: 0.15
Nodes (11): Manage saved sessions., sessions(), CompressionConfig, VerbatimCompactor — compress text while preserving code structure., ConversationSummarizer, Message, Summarize conversation turns for compression using high-density structured templ, Generate a high-density structured compaction template preserving key context in (+3 more)
...
## Knowledge Gaps
- **338 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `context-manager-cli`, `run.sh script`, `COLORTERM` (+333 more)
...
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_resizer.py`, `test_tui_tool_cards.py`, `test_tui_plan_panel.py`, `AutonomousHarness`, `CommandPalette`, `LlamaCppClient`, `RLMEngineOptimized`, `Step`, `tui_app.py`, `PaneResizer`, `PromptTextArea`, `MessageCard`, `test_tui_diff_view.py`, `test_tui_accessibility.py`, `GitFileTree`, `.open_file_tab`, `CenterEmptyState`, `ToolCallCard`, `Static`, `ConnectionPill`, `test_tui_theme.py`, `on`?**
  _High betweenness centrality (0.124) - this node is a cross-community bridge._
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `SymbolIndex`, `tui_app.py`, `rlm_engine_optimized.py`, `PaneResizer`, `ExecutionFeedbackLoop`, `HtmlGamePlayer`, `Static`, `TorchlightApp`, `cli/main.py`, `TestRunResult`, `StreamingChatSession`, `on`, `TieredMemory`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `test_dedup.py`, `test_plan_execution_loop.py`, `REPLSandbox`, `test_resizer.py`, `ProjectMemory`, `test_tui_tool_cards.py`, `test_tui_plan_panel.py`, `DebateVerifier`, `LlamaCppClient`, `context_manager/compression/summarizer.py`, `Step`, `tui_app.py`, `rlm_engine_optimized.py`, `PaneResizer`, `main_optimized.py`, `ExecutionFeedbackLoop`, `MessageCard`, `test_tui_diff_view.py`, `context_manager/memory/models.py`, `.solve_async`, `ToolResult`, `context_manager/memory/persistence.py`, `ProjectMemory`, `TorchlightApp`, `test_inline_interception.py`, `Static`, `on`, `TieredMemory`?**
...
- **Are the 26 inferred relationships involving `RLMEngineOptimized` (e.g. with `ConversationSummarizer` and `Message`) actually correct?**
  _`RLMEngineOptimized` has 26 INFERRED edges - model-reasoned connections that need verification._
