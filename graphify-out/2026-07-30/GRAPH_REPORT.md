...
### Community 31 - "Step"
Cohesion: 0.10
Nodes (23): _beam_budget(), Return (max_beam_files, max_lines_per_file) for the given context size., ConversationSummarizer, Summarizer with LLM-powered and rule-based fallback paths.      When an llm_clie, _build_excerpt(), LLMStateExtractor, _merge_into_state(), _parse_json_response() (+15 more)
...
### Community 38 - "on"
Cohesion: 0.07
Nodes (12): DirectorySelected, on, Pressed, ApprovalModal, CopySelectionModal, FolderPickerModal, Modal dialog for tool & file modification approval., Modal dialog for interactive visual folder selection across the entire computer. (+4 more)
...
Cohesion: 0.13
Nodes (22): _extract_dep_installs(), _extract_failing_tests(), ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject, Enum (+14 more)
...
Cohesion: 0.08
Nodes (37): AutonomousHarness, GoalSpec, HarnessConfig, Enum, ExecutionFeedbackLoop, Path, str, Ensure target project has local git repository and persistent memory initialized (+29 more)
...
### Community 156 - ".format_l0_scratchpad"
Cohesion: 0.33
...
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `SymbolIndex`, `TieredMemory`, `test_tui_plan_panel.py`, `Console`, `on`, `TieredMemory`, `TorchlightApp`, `core/memory/manager.py`, `WebOutcomeInspector`, `TestRunResult`, `Static`, `tui_app.py`, `goal`, `StreamingChatSession`, `cli/main.py`, `Step`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `TokenCounter`, `context_manager/memory/persistence.py`, `_EvictingDeque`, `context_manager/memory/manager.py`, `.clear`, `RLMEngineOptimized`, `build_embedder`, `ProjectMemory`, `AutonomousHarness`, `StreamingChatSession`, `cli/main.py`, `OllamaClient`, `Step`?**
