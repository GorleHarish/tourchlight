...
## Corpus Check
- 156 files · ~239,213 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1943 nodes · 3792 edges · 125 communities (100 shown, 25 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 226 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3c21e984`
...
6. `AutonomousHarness` - 30 edges
7. `get_core_registry()` - 29 edges
8. `Learnings in Building Local SLM Coding Agents` - 28 edges
...
Cohesion: 0.09
Nodes (12): _extract_errors(), _extract_file_paths(), ContextSnapshot, Message, MessageRole, Pin a recently-read file so it survives compression.          If the file is alr, Return list of (path, content) for pinned files., Build context using selective progressive compression.          Kept as a standa (+4 more)

### Community 8 - "AutonomousHarness"
Cohesion: 0.18
Nodes (10): AutonomousHarness, GoalSpec, Return pending tasks whose dependencies are all VERIFIED., Return list of target files that collide with active or failed tasks., Construct inter-task memory prompt summarizing prior verified tasks and dependen, Run a single micro-epoch for a target task., Run continuous autonomous daemon until completion or timeout., Return structured status summary of current goal and sub-agent tasks. (+2 more)

### Community 9 - "implementations.py"
Cohesion: 0.09
Nodes (38): test_list_dir_impl(), test_read_symbols_impl(), Tool risk classification for Torchlight.  Shared between CLI and TUI frontends., _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign (+30 more)
...
Cohesion: 0.06
Nodes (32): ExecutionFeedbackLoop, FileChange, Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-detect test framework from project structure., Run tests and return parsed results., Parse pytest output to extract test results. (+24 more)

### Community 12 - "Flashlight"
Cohesion: 0.20
Nodes (6): _beam_config_for_context(), BeamResult, Flashlight, Flashlight Beam — query-to-code relevance scorer.  Scoring strategy (additive):, FileEntry, Flashlight Indexer — scans the project and builds a searchable symbol index.

### Community 13 - "test_implementations.py"
Cohesion: 0.09
Nodes (31): test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_file_impl(), test_read_file_impl_not_found(), test_run_command_impl(), test_run_command_impl_fail() (+23 more)

### Community 14 - "WebOutcomeInspector"
Cohesion: 0.06
Nodes (32): ExecutionFeedbackLoop, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Detect and run the project's test suite or web inspector., Build feedback context string for the LLM., Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., TestResult (+24 more)

### Community 15 - "TieredMemory"
Cohesion: 0.08
Nodes (13): ContextSnapshot, Pin a recently-read file slice so it survives compression without bloating conte, Return list of (path, content) for pinned files., Remove all pinned files., Compress older messages, preserving the first N messages., Async wrapper for compress_recent., Build the message list for the LLM.          Pinned files are injected as a syst, Build critical context block from session state. (+5 more)

### Community 16 - "SelectiveCompressor"
Cohesion: 0.17
Nodes (9): CompressionConfig, Pattern, Progressive compression that preserves semantic meaning.      Strategy:     - Re, Compress a list of messages using progressive levels., SelectiveCompressor, TurnSummary, test_selective_compressor_custom_config(), test_selective_compressor_defaults() (+1 more)
...
Cohesion: 0.11
Nodes (22): ensure_git_repository(), ensure_project_initialized(), init_new_project(), ProjectMemory, Path, SessionState, Session and project memory persistence for Torchlight., Ensure target project directory exists and has `.context-memory.json` persistent (+14 more)
...
### Community 21 - "core/tests/test_models.py"
Cohesion: 0.11
Nodes (27): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), ContentChunk (+19 more)
...
Cohesion: 0.07
Nodes (28): 24-Hour Continuous Autonomous Execution & Micro-Epoch Context Flushing, 7B Model EDIT_FILE Failure Modes, Active File Pinning for Context Preservation, Aider-Style Search/Replace Diff Blocks vs JSON `old_text`, Consume-on-Read Pattern for Automated Feedback Context, Context Loss & Memory Compression, Context Recovery via Dynamic JIT File Pinning Budget Scaling, Context Window Sizing for 8GB Devices (+20 more)
...
Cohesion: 0.07
Nodes (27): 2026-07-23, 2026-07-24, 2026-07-26, 2026-07-27, 24-Hour Continuous Autonomous Goal Harness (`AutonomousHarness`), Active File Pinning (Context Management Fix), Automatic Project & Persistent Memory File Initialization, Changelog (+19 more)

### Community 31 - "rlm_engine_optimized.py"
Cohesion: 0.33
Nodes (6): MemoryConfig, test_tiered_memory_pinned_files_without_system_message(), _clean_and_parse_json(), Parse the LLM response for action tags.         Returns: (action, thinking, cont, SolveResult, Step
...
Cohesion: 0.13
Nodes (23): _extract_dep_installs(), _extract_failing_tests(), MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle (+15 more)

### Community 35 - "config.py"
Cohesion: 0.11
Nodes (16): _detect_apple_silicon_ram(), _detect_chip(), fetch_provider_models(), is_port_in_use(), list_available_models(), normalize_model_name(), Normalize model alias names (e.g. 'gemma-2-2b', 'qwen', 'gemma 4 E2B')., Scan local models directory and returns available GGUF and MLX models. (+8 more)
...
Cohesion: 0.18
Nodes (7): _EvictingDeque, TokenCounter, Deque that fires a callback when an item is evicted due to maxlen., Validate all tracked file paths against the actual filesystem.         Prunes no, Minimum NEW tokens that must arrive before re-compression is allowed.          S, deque, MemoryObject
...
### Community 43 - "RLMEngine"
Cohesion: 0.10
Nodes (18): _build_excerpt(), LLMStateExtractor, _merge_into_state(), _parse_json_response(), LLM-powered SessionState extractor.  Replaces the regex-based _merge_summary_int, Robustly extract a JSON object from the model's response.      Local models some, Merge the extracted JSON fields into the existing SessionState.      Strategy: L, Uses the local LLM to extract structured SessionState fields from a     conversa (+10 more)
...
Cohesion: 0.33
Nodes (6): Tests for performance and accuracy optimizations in Torchlight., test_batch_tool_execution(), test_symbol_index_mtime_cache(), test_get_tool_registry(), get_tool_registry(), Get or create the global tool registry.
...
Cohesion: 0.16
Nodes (12): command, compress_file(), count_tokens(), Compress a file using verbatim compaction., Count tokens in text., Manage saved sessions., _risk_tier(), sessions() (+4 more)
...
Cohesion: 0.17
Nodes (14): Tests for Aider-style Search/Replace block editing (Approach B) and dynamic JIT, test_edit_file_diff_block_in_old_text(), test_edit_file_with_diff_block(), test_parse_diff_block_invalid(), test_parse_diff_block_valid(), test_pin_file_truncation_to_budget(), test_edit_file_impl(), test_edit_file_impl_not_found() (+6 more)
...
Cohesion: 0.29
Nodes (10): test_classify_confirm_commands(), test_classify_destructive_commands(), test_classify_empty_command(), test_classify_safe_commands(), test_classify_unknown_defaults_to_confirm(), test_classify_whitespace_handling(), classify_command(), classify_tool() (+2 more)
...
Cohesion: 0.40
Nodes (3): ExecutionFeedbackLoop, Path, Ensure target project has local git repository and persistent memory initialized
...
Cohesion: 0.31
Nodes (6): HarnessConfig, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, TaskStatus, main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., str
...
Cohesion: 0.21
Nodes (5): Path, SymbolIndex, test_file_entry(), test_symbol_index_build(), test_symbol_index_summary()
...
## Knowledge Gaps
- **61 isolated node(s):** `graphify`, `Workflow: graphify`, `Key Classes & Functions`, `Ephemeral Web Outcome Inspection Subsystem (`WebOutcomeInspector` & `INSPECT_WEB`)`, `Performance & Accuracy Optimization Suite` (+56 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `AutonomousHarness`, `.__init__`, `RLMEngine`, `autonomous_harness.py`, `test_autonomous_harness.py`, `SelectiveCompressor`, `core/memory/__init__.py`, `test_autonomous_harness_pipeline.py`, `core/tests/test_models.py`, `test_context_budget_overflow.py`, `RLMEngineOptimized`, `test_diff_edit.py`, `rlm_engine_optimized.py`?**
