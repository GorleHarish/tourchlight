...
## Corpus Check
- 161 files · ~245,750 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2384 nodes · 4540 edges · 149 communities (131 shown, 18 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 344 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `79dfe2b2`
...
Cohesion: 0.11
Nodes (27): test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_run_command_impl(), test_run_command_impl_fail() (+19 more)
...
Cohesion: 0.12
Nodes (29): _extract_dep_installs(), _extract_failing_tests(), MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle (+21 more)
...
Cohesion: 0.10
Nodes (25): _extract_symbols(), Return (MAX_LINES, MAX_CHARS) for the current context window., Return [(lineno_1based, kind, name), ...] sorted by line number., Compact symbol map prepended to READ_FILE output., READ_FILE — read a file with optional line-range or symbol syntax.      Formats:, READ_SYMBOLS — show the structure of a file without loading its content.      Re, _read_budget(), _symbol_map() (+17 more)
...
Cohesion: 0.16
Nodes (25): Conversation Summarizer for Torchlight.  Extracts key information from conversat, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject, Message (+17 more)
...
### Community 84 - "ConversationSummarizer"
Cohesion: 0.29
...
- **Why does `TieredMemory` connect `TieredMemory` to `TokenCounter`, `build_embedder`, `.clear`, `context_manager/memory/persistence.py`, `LLMStateExtractor`, `context_manager/memory/manager.py`, `_EvictingDeque`, `cli/main.py`, `ProjectMemory`, `AutonomousHarness`, `StreamingChatSession`, `.__init__`, `RLMEngine`, `RLMEngineOptimized`?**
  _High betweenness centrality (0.070) - this node is a cross-community bridge._
- **Why does `StreamingChatSession` connect `StreamingChatSession` to `context_manager/compression/summarizer.py`, `LMStudioClient`, `._update_params`, `ActionTracker`, `context_manager/compression/compactor.py`, `TieredMemory`, `ExecutionFeedbackLoop`, `context_manager/memory/manager.py`, `test_phase_detection.py`, `VerbatimCompactor`, `cli/main.py`, `._execute_tool_with_approval`, `ProjectMemory`, `AutonomousHarness`, `ContextDashboard`, `DebateVerifier`, `.__init__`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `context_manager/compression/summarizer.py`, `tui_app.py`, `Static`, `ApprovalModal`, `rlm_engine_optimized.py`, `TieredMemory`, `on`, `TorchlightApp`, `ExecutionFeedbackLoop`, `context_manager/memory/manager.py`, `ProjectMemory`, `REPLSandbox`, `ProjectMemory`, `LlamaCppClient`, `DebateVerifier`?**
