...
## Corpus Check
- 162 files · ~248,019 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2415 nodes · 4367 edges · 159 communities (135 shown, 24 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 233 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dab76c48`
...
2. `TieredMemory` - 60 edges
3. `TorchlightApp` - 55 edges
4. `RLMEngineOptimized` - 34 edges
5. `Learnings in Building Local SLM Coding Agents` - 34 edges
6. `AutonomousHarness` - 33 edges
7. `ProjectSnapshot` - 31 edges
8. `SkillResult` - 31 edges
9. `get_core_registry()` - 29 edges
10. `BaseSkill` - 28 edges

## Surprising Connections (you probably didn't know these)
- `RLMEngineOptimized` --uses--> `MemoryConfig`  [INFERRED]
  rlm_optimized/rlm_engine_optimized.py → context-manager-cli/src/context_manager/memory/manager.py
...
## Communities (159 total, 24 thin omitted)
...
### Community 1 - "context_manager/memory/persistence.py"
Cohesion: 0.16
...
Cohesion: 0.11
Nodes (8): App, Panel, Mount a widget defensively and scroll safely after layout pass., Build the AST knowledge graph for the current project_root in a         backgrou, Manually trigger memory context compaction., Codex / Tiny-Brain 2 Style Agent IDE TUI., TorchlightApp, work
...
Cohesion: 0.06
Nodes (24): EphemeralHTTPServer, Any, Path, QuietHTTPRequestHandler, Web Outcome Inspector for Torchlight.  Provides low-memory, ephemeral runtime an, Tier 1: Static HTML syntax and asset path validator., Tier 3: Run Node JSDOM script if node is available., Tier 1: Static HTML analysis using Python standard library. (+16 more)
...
Cohesion: 0.19
Nodes (6): chat(), Run out-of-band DebateVerifier pass if candidate proposal needs verification., Build the final message list for the LLM, respecting the context budget., /params                    — show current params         /params auto, Start an interactive chat session with context management and flashlight.      I, StreamingChatSession
...
Cohesion: 0.16
Nodes (11): MemoryConfig, TokenCounter, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Unit tests for manual context compaction trigger and 85%/91% threshold logic., test_core_compress_recent_force(), test_core_should_compress_high_ratio_low_message_count(), test_engine_compact_context(), Tests for read/edit tool memory synchronization (unpin_file, refresh_pin). (+3 more)
...
### Community 40 - ".append"
Cohesion: 0.20
Nodes (6): _extract_dep_installs(), _extract_errors(), _extract_failing_tests(), _extract_file_paths(), MemoryNeedle, MessageRole

### Community 41 - "_detect_apple_silicon_ram"
Cohesion: 0.09
Nodes (12): _detect_apple_silicon_ram(), _detect_chip(), fetch_provider_models(), list_available_models(), normalize_model_name(), Normalize model alias names (e.g. 'gemma-2-2b', 'qwen', 'gemma 4 E2B')., Scan local models directory and returns available GGUF and MLX models., Detect Apple Silicon chip name (e.g., 'Apple M1'). (+4 more)
...
Cohesion: 0.18
Nodes (16): ContentChunk, ContentType, ContextSnapshot, Message, Enum, SessionState, WorkingSetSnapshot, test_content_chunk_custom() (+8 more)
...
Cohesion: 0.10
Nodes (22): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Run fast pre-flight auto-fixer/linter on modified files before test execution. (+14 more)
...
### Community 52 - "context_manager/memory/embeddings.py"
Cohesion: 0.19
...
Cohesion: 0.17
Nodes (14): MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, create_client(), display_step(), get_depth_style(), main(), print_banner(), print_help() (+6 more)
...
### Community 65 - "ConversationSummarizer"
Cohesion: 0.25
...
Cohesion: 0.15
Nodes (15): test_write_file_impl(), test_write_file_impl_missing_path(), Tests for performance and accuracy optimizations in Torchlight., test_batch_tool_execution(), test_inline_syntax_guardrail(), test_symbol_index_mtime_cache(), test_get_tool_registry(), test_tool_registry_preview_dry_run() (+7 more)
...
### Community 72 - "Step"
Cohesion: 0.12
Nodes (5): Step, ApprovalModal, ModelPickerModal, Modal dialog for tool & file modification approval., Modal dialog to visually pick models and engine providers.
...
### Community 110 - "test_phase_detection.py"
Cohesion: 0.21
...
### Community 126 - "main_optimized.py"
Cohesion: 0.29
...
Cohesion: 0.10
Nodes (29): ExecutionFeedbackLoop, Closes the loop between code changes and test verification.          Flow:     1, AutonomousHarness, GoalSpec, HarnessConfig, Enum, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Return pending tasks whose dependencies are all VERIFIED. (+21 more)
...
## Knowledge Gaps
- **365 isolated node(s):** `Manual Context Compaction Trigger & High-Ratio Threshold Fix (`core/memory/manager.py`, `context-manager-cli`, `rlm_optimized`)`, `Flexible Diff Parsing & Diagnostic Error Hints for `EDIT_FILE` (`implementations.py`, `test_diff_edit.py`)`, `Industry-Standard Non-Verbose Code Output & Context Protection Protocol (`system.py`, `prompts.py`, `AGENTS.md`)`, `Real-Time Implementation Plan TUI Sidebar Panel (`tui_app.py`, `tui_app.tcss`)`, `Native AST Knowledge Graph Engine (`graph_engine.py` & `SEARCH_AST`)` (+360 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `deque`, `.append`, `_EvictingDeque`, `Step`, `cli/main.py`, `Message`, `AutonomousHarness`, `.__init__`, `.clear`, `StreamingChatSession`, `RLMEngine`, `RLMEngineOptimized`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `BaseSkill`, `TDDSkill`, `MarkdownDocumentSkill`, `unified.py`, `PlanningSkill`, `discovery.py`, `TDDSkill`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `BaseSkill` connect `BaseSkill` to `ProjectSnapshot`, `TDDSkill`, `MarkdownDocumentSkill`, `SkillResult`, `unified.py`, `PlanningSkill`, `TDDSkill`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Are the 14 inferred relationships involving `TieredMemory` (e.g. with `sessions()` and `StreamingChatSession`) actually correct?**
...
- **What connects `Manual Context Compaction Trigger & High-Ratio Threshold Fix (`core/memory/manager.py`, `context-manager-cli`, `rlm_optimized`)`, `Flexible Diff Parsing & Diagnostic Error Hints for `EDIT_FILE` (`implementations.py`, `test_diff_edit.py`)`, `Industry-Standard Non-Verbose Code Output & Context Protection Protocol (`system.py`, `prompts.py`, `AGENTS.md`)` to the rest of the system?**
