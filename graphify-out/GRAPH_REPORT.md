...
## Corpus Check
- 165 files · ~245,364 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2452 nodes · 4402 edges · 166 communities (138 shown, 28 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 202 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `63896807`
...
Cohesion: 0.10
Nodes (11): App, Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline., Mount a widget defensively and scroll safely after layout pass., Build the AST knowledge graph for the current project_root in a         backgrou, Manually trigger memory context compaction., Codex / Tiny-Brain 2 Style Agent IDE TUI., Extract text from the TextArea, clear it, and dispatch. (+3 more)
...
Cohesion: 0.10
Nodes (32): _extract_dep_installs(), _extract_failing_tests(), MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle (+24 more)
...
Cohesion: 0.12
Nodes (18): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Run fast pre-flight auto-fixer/linter on modified files before test execution. (+10 more)
...
Cohesion: 0.08
Nodes (25): ContextSnapshot, MemoryConfig, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Return list of (path, content) for pinned files., Calculate remaining token budget headroom before reaching max_tokens threshold., Predict likely next tools based on current state., Tiered memory system with L0-L3 hierarchy:     - L0: Active prompt (current cont, TieredMemory (+17 more)
...
Cohesion: 0.16
Nodes (12): copy_to_clipboard(), CopySelectionModal, create_client(), load_last_state(), main(), _provider_runtime_info(), Torchlight Agent — Codex / Tiny-Brain 2 Style IDE TUI (Textual) Full-featured ID, Return (port, externally_managed) for a given provider key.      externally_mana (+4 more)
...
Cohesion: 0.17
Nodes (14): test_rlm_engine_solve_method(), test_rlm_engine_unwrapped_thought_parsing(), create_client(), display_step(), get_depth_style(), main(), print_banner(), print_help() (+6 more)
...
### Community 69 - ".append"
Cohesion: 0.16
Nodes (6): _extract_errors(), _extract_file_paths(), MessageRole, Pin a recently-read file so it survives compression.          If the file is alr, Re-read an edited file from disk and update its pin in memory., MemoryNeedle
...
### Community 123 - "get_tool_registry"
Cohesion: 0.25
Nodes (8): Tests for performance and accuracy optimizations in Torchlight., test_batch_tool_execution(), test_inline_syntax_guardrail(), test_symbol_index_mtime_cache(), test_get_tool_registry(), test_tool_registry_preview_dry_run(), get_tool_registry(), Get or create the global tool registry.
...
### Community 134 - "autonomous_harness.py"
Cohesion: 0.18
Nodes (11): test_render_task_progress_empty(), test_render_task_progress_with_tasks(), GoalSpec, HarnessConfig, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Return structured status summary of current goal and sub-agent tasks., TaskStatus, main() (+3 more)
...
### Community 138 - "context_manager/execution/feedback_loop.py"
Cohesion: 0.39
...
### Community 148 - "test_autonomous_harness_pipeline.py"
Cohesion: 0.43
...
### Community 154 - ".format_l0_scratchpad"
Cohesion: 0.33
...
## Knowledge Gaps
- **325 isolated node(s):** `Added & Improved`, `Added & Improved`, `Fixed & Improved`, `Added & Improved`, `Added & Improved` (+320 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `cli/main.py`, `autonomous_harness.py`, `core/memory/__init__.py`, `Message`, `SelectiveCompressor`, `.__init__`, `test_autonomous_harness_pipeline.py`, `AutonomousHarness`, `.__init__`, `test_context_budget_overflow.py`, `.refresh_pin`, `.format_l0_scratchpad`, `StreamingChatSession`?**
...
- **Why does `TieredMemory` connect `TieredMemory` to `.append`, `TokenCounter`, `LLMStateExtractor`, `context_manager/memory/manager.py`, `_EvictingDeque`, `ProjectMemory`, `build_embedder`?**
...
- **What connects `Added & Improved`, `Added & Improved`, `Fixed & Improved` to the rest of the system?**
