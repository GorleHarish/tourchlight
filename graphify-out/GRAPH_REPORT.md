...
## Corpus Check
- 222 files · ~156,506 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3235 nodes · 5967 edges · 210 communities (176 shown, 34 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 300 edges (avg confidence: 0.58)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a773d1c8`
...
## Communities (210 total, 34 thin omitted)

### Community 0 - "implementations.py"
Cohesion: 0.07
Nodes (53): Core Tools — built-in, always available, called via TOOL_NAME("arg") syntax.  Th, test_list_dir_impl(), test_read_file_impl(), test_read_file_impl_not_found(), _auto_repair(), _ddg_search(), _detect_doc_source(), _extract_identifiers() (+45 more)
...
Cohesion: 0.11
Nodes (25): ensure_git_repository(), ensure_project_initialized(), init_new_project(), Path, Session and project memory persistence for Torchlight., Ensure target project directory exists and has a local Git repository initialize, Write a marker proving the harness itself initialized this git repo.      Only w, Ensure target project directory exists and has `.context-memory.json` persistent (+17 more)
...
Cohesion: 0.19
Nodes (11): test_search_ast_schema_validation(), test_validate_tool_call_with_unwrapping_and_coercion(), _coerce_param(), get_openai_tools_schema(), get_schemas_for_phase(), Tool schemas and validation for Torchlight.  Defines OpenAI-compatible JSON sche, Coerce LLM-provided values to expected schema types., Validate a tool call against its schema and resolve parameter aliases.      Retu (+3 more)
...
Cohesion: 0.13
Nodes (24): ContentChunk, ContentType, ContextSnapshot, ExecutionMode, MemoryNeedle, Message, MessageRole, Enum (+16 more)
...
Cohesion: 0.18
Nodes (10): AgentMemoryWidget, create_client(), load_last_state(), main(), Torchlight Agent — Codex / Tiny-Brain 2 Style IDE TUI (Textual) Full-featured ID, Modal dialog displaying keyboard shortcuts and slash commands., Displays the live L0 Agent Brain Scratchpad., # NOTE: The model badge button click is handled by on_model_badge_clicked (+2 more)
...
Cohesion: 0.11
Nodes (29): build_embedder(), compute_tf_idf_score(), cosine_similarity(), Embedder, HybridEmbedder, HybridMemoryRetriever, _is_low_memory(), KeywordEmbedder (+21 more)
...
### Community 66 - "CopySelectionModal"
Cohesion: 0.12
...
Cohesion: 0.08
Nodes (10): copy_to_clipboard(), _provider_runtime_info(), Copy text to system clipboard across macOS, Linux, and Windows., Return (port, externally_managed) for a given provider key.      externally_mana, Sync all connection-dependent UI elements.          Called on server status chan, Build the AST knowledge graph silently in background thread., Consolidated Phase-4 status bar (state · model · context gauge · tps · tokens ·, Repoint the file tree at the engine root and refresh git status. (+2 more)

### Community 70 - "test_code_quality_harness.py"
Cohesion: 0.10
Nodes (35): Unit tests for Torchlight Zero-Context Code Quality Harness., test_check_syntax_js_bracket_balance(), test_check_syntax_js_string_literal_brackets(), test_check_syntax_json(), test_check_syntax_python(), test_compile_gate_rejects_return_outside_function(), test_detect_symptom_patching(), test_edit_file_blocks_broken_syntax() (+27 more)
...
Cohesion: 0.06
Nodes (11): App, ContextFileAttached, FileSelected, NodeSelected, Codex / Tiny-Brain 2 Style Agent IDE TUI., Update the context usage bar in the Agent tab., Show or hide the center empty state (hide when a file is open)., Global key bindings that aren't caught by specific widgets.          Key contrac (+3 more)

### Community 83 - "setup_optimized.sh"
Cohesion: 0.60
Nodes (3): info(), ok(), setup_optimized.sh script

### Community 84 - "._submit_user_input"
Cohesion: 0.23
...
### Community 105 - "test_autonomous_harness.py"
Cohesion: 0.26
...
Cohesion: 0.05
Nodes (53): ContextSnapshot, MemoryConfig, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Tiered memory system with L0-L3 hierarchy:     - L0: Active prompt (current cont, Return list of (path, content) for pinned files., Calculate remaining token budget headroom before reaching max_tokens threshold., Predict likely next tools based on current state., TieredMemory (+45 more)
...
Cohesion: 0.08
Nodes (36): anyio, test_get_workspace_pending_tasks_goal_spec(), test_get_workspace_pending_tasks_md(), test_sync_workspace_tasks_populates_tasks_md(), test_verification_gate_allows_final_answer_when_all_done(), test_verification_gate_rejects_premature_final_answer(), _build_plan_text(), _make_app() (+28 more)
...
### Community 138 - "CenterEmptyState"
Cohesion: 0.18
...
Cohesion: 0.08
Nodes (27): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), test_list_available_models_includes_gemma4e4b() (+19 more)
...
### Community 167 - "._handle_step"
Cohesion: 0.19
Nodes (4): Append a line to the Output tab's RichLog.          severity: 'info' | 'tool' |, Mount a widget defensively and scroll safely after layout pass., Mount a running ToolCallCard for a streamed ``<tool_call>`` marker.          Kep, StreamingView
...
### Community 171 - "sanitize_assistant_text"
Cohesion: 0.18
...
### Community 178 - "tool_schemas.py"
Cohesion: 0.29
Nodes (6): test_get_openai_tools_schema(), get_openai_tools_schema(), Schema definitions and validation for SLM tool calls. Ensures Small Language Mod, Generate OpenAI-style tools list for function calling / structured schema enforc, Validate tool call against schema and normalize parameter aliases.          Retu, validate_and_normalize_tool_call()

### Community 179 - "TestApp"
Cohesion: 0.22
Nodes (5): App, ComposeResult, on, Pressed, TestApp

### Community 180 - "iter_project_files"
Cohesion: 0.27
...
Cohesion: 0.09
Nodes (15): is_valid_file_path(), Persist L0 working state to disk in .context-memory.json (debounced)., Update or set the primary system prompt (first system message in history)., Remove all pinned files., Generic add_message method for role-based message addition., Compress older messages, preserving the first N messages., Async wrapper for compress_recent., Compact context between tasks while preserving continuous session state. (+7 more)

### Community 183 - "CompressionConfig"
Cohesion: 0.22
Nodes (6): CompressionConfig, Configuration for selective memory compression., _is_valid_decision(), Load persistent project memory (.context-memory.json) into L0 working state., Filter out empty, generic, or noisy session summary strings., TokenCounter

### Community 184 - "test_context_budget_overflow.py"
Cohesion: 0.40
...
- **Why does `TieredMemory` connect `TieredMemory` to `test_tui_plan_panel.py`, `OllamaClient`, `rlm_engine_optimized.py`, `AutonomousHarness`, `Step`, `StreamingChatSession`, `RLMEngineOptimized`, `cli/main.py`, `core/memory/models.py`, `test_autonomous_harness_pipeline.py`, `context_manager/memory/manager.py`, `._update_state_from_message`, `CompressionConfig`, `.format_l0_scratchpad`, `tool_edit_file_impl`, `test_context_budget_overflow.py`, `test_enhanced_memory_retrieval.py`, `.compact_context`, `test_autonomous_harness.py`?**
  _High betweenness centrality (0.129) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `test_tui_plan_panel.py`, `test_resizer.py`, `rlm_engine_optimized.py`, `Step`, `test_tui_tool_cards.py`, `Static`, `on`, `PaneResizer`, `main_optimized.py`, `._parse_response`, `test_tui_transcript_widgets.py`, `ApprovalModal`, `test_tui_diff_view.py`, `tui_app.py`, `test_goal_mode_process.py`, `._stream_llm`, `ToolResult`, `.compact_context`, `CopySelectionModal`, `TorchlightApp`, `test_inline_interception.py`, `test_autonomous_harness.py`, `test_tui_trajectory_rail.py`, `TieredMemory`?**
  _High betweenness centrality (0.114) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_tui_plan_panel.py`, `test_resizer.py`, `Step`, `test_tui_tool_cards.py`, `RLMEngineOptimized`, `on`, `._handle_step`, `PaneResizer`, `sanitize_assistant_text`, `test_tui_transcript_widgets.py`, `ApprovalModal`, `test_tui_diff_view.py`, `tui_app.py`, `test_tui_accessibility.py`, `CopySelectionModal`, `._handle_slash_command`, `._submit_user_input`, `test_tui_trajectory_rail.py`, `test_tui_theme.py`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Are the 3 inferred relationships involving `TorchlightApp` (e.g. with `_StubClient` and `RLMEngineOptimized`) actually correct?**
...
- **Are the 16 inferred relationships involving `RLMEngineOptimized` (e.g. with `MemoryConfig` and `TieredMemory`) actually correct?**
  _`RLMEngineOptimized` has 16 INFERRED edges - model-reasoned connections that need verification._
