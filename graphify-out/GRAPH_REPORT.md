...
## Corpus Check
- 166 files · ~119,653 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2474 nodes · 4626 edges · 157 communities (131 shown, 26 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 296 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0a0187bd`
...
Cohesion: 0.12
Nodes (25): _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, WEB_SEARCH — general web search., DOC_SEARCH — search official documentation., WEB_VERIFY — verify code snippet API calls against documentation. (+17 more)
...
Cohesion: 0.09
Nodes (34): VerbatimCompactor — compress text while preserving code structure., ConversationSummarizer, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Extract key information from text., ContentChunk, ContentType, ContextSnapshot (+26 more)
...
Cohesion: 0.11
Nodes (27): Recovery engine for Torchlight errors.  Provides structured recovery strategies, Tracks retry state for a specific error pattern., Decide what to do after an error.          Returns a RecoveryAction indicating t, RecoveryState, ConnectionError, ContextOverflowError, ParseError, Enum (+19 more)

### Community 13 - "test_implementations.py"
Cohesion: 0.09
Nodes (31): test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_list_dir_impl(), test_read_symbols_impl() (+23 more)

### Community 14 - "WebOutcomeInspector"
Cohesion: 0.09
Nodes (22): EphemeralHTTPServer, Any, HTMLParser, Path, QuietHTTPRequestHandler, Web Outcome Inspector for Torchlight.  Provides low-memory, ephemeral runtime an, Tier 1: Static HTML syntax and asset path validator., Main Inspector Subsystem driving zero-memory, ephemeral web verification. (+14 more)

### Community 15 - "RLMEngineOptimized"
Cohesion: 0.09
Nodes (22): MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, Message, MessageRole, test_rlm_engine_code_tag_and_backticks(), test_rlm_engine_debate_verifier_error_resilience(), test_rlm_engine_optimized_code_execution(), test_rlm_engine_optimized_debate_verifier_initialization() (+14 more)
...
Cohesion: 0.07
Nodes (28): ActionTracker, chat(), get_phase_system_prompt(), Start an interactive chat session with context management and flashlight.      I, Estimate tokens consumed by system prompt, tools, and flashlight beam., Infer the current agent phase from user input and the last model response., Auto-switch _params based on detected phase.  No-op when locked., Run out-of-band DebateVerifier pass if candidate proposal needs verification. (+20 more)
...
Cohesion: 0.12
Nodes (23): _extract_dep_installs(), _extract_failing_tests(), _extract_tech_stack(), ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject (+15 more)
...
Cohesion: 0.10
Nodes (23): Post-edit auto-run test suite failure., TestFailureError, ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback (+15 more)
...
Cohesion: 0.05
Nodes (44): command, _beam_budget(), compress_file(), count_tokens(), Compress a file using verbatim compaction., Count tokens in text., Manage saved sessions., Return (max_beam_files, max_lines_per_file) for the given context size. (+36 more)

### Community 51 - "tool_read_file_impl"
Cohesion: 0.11
Nodes (23): _extract_symbols(), Return [(lineno_1based, kind, name), ...] sorted by line number., Compact symbol map prepended to READ_FILE output., READ_FILE — read a file with optional line-range or symbol syntax.      Formats:, READ_SYMBOLS — show the structure of a file without loading its content.      Re, _symbol_map(), tool_read_file(), tool_read_symbols() (+15 more)
...
### Community 72 - "context_manager/memory/persistence.py"
Cohesion: 0.29
...
### Community 104 - "._update_state_from_message"
Cohesion: 0.28
...
### Community 138 - ".format_l0_scratchpad"
Cohesion: 0.33
...
Cohesion: 0.08
Nodes (40): AutonomousHarness, GoalSpec, HarnessConfig, ExecutionFeedbackLoop, Path, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Ensure target project has local git repository and persistent memory initialized, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa (+32 more)
...
## Knowledge Gaps
- **326 isolated node(s):** `tui.sh script`, `COLORTERM`, `TERM`, `PYTHONPATH`, `context-manager-cli` (+321 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `rlm_engine_optimized.py`, `.format_l0_scratchpad`, `.refresh_pin`, `RLMEngineOptimized`, `TokenCounter`, `AutonomousHarness`, `tool_edit_file_impl`, `StreamingChatSession`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `BaseSkill`, `TDDSkill`, `UnifiedSkillRegistry`, `CalculatorSkill`, `MarkdownDocumentSkill`, `PlanningSkill`, `discovery.py`, `TDDSkill`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `LLMStateExtractor`, `TokenCounter`, `._update_state_from_message`, `_EvictingDeque`, `context_manager/memory/manager.py`, `RLMEngineOptimized`, `build_embedder`, `ProjectMemory`, `.clear`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
...
- **Are the 17 inferred relationships involving `TieredMemory` (e.g. with `LLMStateExtractor` and `ContextSnapshot`) actually correct?**
