...
## Corpus Check
- 199 files · ~151,886 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3057 nodes · 5954 edges · 179 communities (154 shown, 25 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 511 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5d97d396`
...
Cohesion: 0.08
Nodes (35): test_list_dir_impl(), _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, Resolve a path relative to project root, handling ~ and absolute paths., LIST_DIR — list directory contents. (+27 more)
...
Cohesion: 0.11
Nodes (20): test_get_tool_registry(), test_tool_registry_execute(), test_tool_registry_execute_unknown(), test_tool_registry_get(), test_tool_registry_preview_dry_run(), test_tool_registry_register(), test_tool_registry_risk_level(), test_tool_registry_risk_level_run_command() (+12 more)
...
Cohesion: 0.13
Nodes (22): test_verify_compile_param(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_file_impl() (+14 more)
...
### Community 31 - "CopySelectionModal"
Cohesion: 0.10
Nodes (16): Horizontal, CopySelectionModal, ComposeResult, Modal dialog to select and copy specific messages, code blocks, or text turns., Modal dialog displaying keyboard shortcuts and slash commands., ShortcutsHelpModal, ComposeResult, Consolidated status bar for the Torchlight TUI.  Phase 4: one glanceable row tha (+8 more)
...
Cohesion: 0.09
Nodes (21): MemoryConfig, get_phase_system_prompt(), Unified system prompts for Torchlight.  Single source of truth for all frontends, Generate phase-tailored system prompt by appending phase instructions., Unit tests for manual context compaction trigger and 85%/91% threshold logic., test_core_compress_recent_force(), test_core_should_compress_high_ratio_low_message_count(), test_engine_compact_context() (+13 more)
...
Cohesion: 0.07
Nodes (22): ContextSnapshot, Message, Tiered memory system with L0-L3 hierarchy:     - L0: Active prompt (current cont, Persist L0 working state to disk in .context-memory.json., Return list of (path, content) for pinned files., Remove all pinned files., Compress older messages, preserving the first N messages., Async wrapper for compress_recent. (+14 more)
...
Cohesion: 0.10
Nodes (27): _extract_dep_installs(), _extract_failing_tests(), ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject, Enum (+19 more)
...
Cohesion: 0.15
Nodes (17): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_read_symbols_indented_methods_and_duplicate_names(), test_run_command_intercept_ast_functions(), test_search_ast_action_aliases(), test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_read_symbols_impl(), test_run_command_impl() (+9 more)
...
Cohesion: 0.08
Nodes (30): test_list_available_models_includes_gemma4e4b(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), test_normalize_mlx_gemma_4_4e4b(), fetch_provider_models(), list_available_models(), normalize_model_name(), Normalize model alias names (e.g. 'gemma-2-2b', 'qwen', 'gemma 4 E2B', 'gemma 4 (+22 more)
...
Cohesion: 0.24
Nodes (16): HarnessConfig, main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for AutonomousHarness module., A task that made no code changes must not be reverted by unrelated /     pre-exi (+8 more)

### Community 70 - "test_code_quality_harness.py"
Cohesion: 0.11
Nodes (31): Unit tests for Torchlight Zero-Context Code Quality Harness., test_check_syntax_js_bracket_balance(), test_check_syntax_js_string_literal_brackets(), test_check_syntax_json(), test_check_syntax_python(), test_compile_gate_rejects_return_outside_function(), test_edit_file_blocks_broken_syntax(), test_force_bypasses_validation_gates() (+23 more)
...
Cohesion: 0.07
Nodes (29): ConversationSummarizer, Summarizer with LLM-powered and rule-based fallback paths.      When an llm_clie, MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, Message, MessageRole, test_rlm_engine_solve_method(), LlamaCppClient (+21 more)
...
Cohesion: 0.17
Nodes (12): command, compress_file(), count_tokens(), goal(), Start an autonomous goal execution session driven by .torchlight task tracking., Compress a file using verbatim compaction., Count tokens in text., Manage saved sessions. (+4 more)
...
Cohesion: 0.11
Nodes (17): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), _detect_apple_silicon_ram() (+9 more)
...
Cohesion: 0.23
Nodes (9): Return True only if a run succeeded. Uses exit code as the authoritative, TestRunResult, create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for Inter-Task Context Pipeline, Dependencies, and File Collision Gua, test_inter_task_output_summary_injection(), test_target_file_collision_detection() (+1 more)
...
Cohesion: 0.09
Nodes (11): _extract_errors(), _extract_file_paths(), ContextSnapshot, Message, MessageRole, Return list of (path, content) for pinned files., Build context using selective progressive compression.          Kept as a standa, Build a compact dev-session state summary injected at context head. (+3 more)
...
Cohesion: 0.10
Nodes (21): AutonomousHarness, GoalSpec, Enum, Path, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Ensure target project has local git repository and persistent memory initialized, Ensure a goal spec exists on disk in .torchlight, initializing a default workspa, Return pending tasks whose dependencies are all VERIFIED. (+13 more)
...
### Community 157 - "classify_command"
Cohesion: 0.22
...
Cohesion: 0.06
Nodes (40): ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Freshly verify any modified-but-unverified files and return True if         ever (+32 more)
...
Cohesion: 0.13
Nodes (8): AgentStatusModal, Modal dialog for complete visibility into background agent actions & status tele, PromptTextArea, Message, TextArea whose Enter submits instead of inserting a newline.      Hooks ``update, Posted when the user presses Enter with no active suggestion., SubmitRequested, TextArea
...
Cohesion: 0.05
Nodes (22): App, FileSelected, NodeSelected, is_port_in_use(), Check if server port 8080 is actively listening., copy_to_clipboard(), _provider_runtime_info(), Step (+14 more)

## Knowledge Gaps
- **368 isolated node(s):** `Added & Improved`, `Added & Improved`, `Added & Improved`, `Added & Improved`, `Added & Improved` (+363 more)
...
- **Why does `AutonomousHarness` connect `AutonomousHarness` to `test_autonomous_harness.py`, `MemoryConfig`, `.__init__`, `cli/main.py`, `ExecutionFeedbackLoop`, `TieredMemory`, `core/memory/manager.py`, `on`, `TorchlightApp`, `PromptTextArea`, `tui_app.py`, `StreamingChatSession`, `TestRunResult`, `CopySelectionModal`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `TieredMemory`, `test_tui_plan_panel.py`, `ExecutionFeedbackLoop`, `ProjectMemory`, `REPLSandbox`, `ProjectMemory`, `test_plan_execution_loop.py`, `DebateVerifier`, `.solve_async`, `CopySelectionModal`, `MemoryConfig`, `on`, `TieredMemory`, `MessageCard`, `test_tui_diff_view.py`, `PromptTextArea`, `TrajectoryRail`, `tui_app.py`, `test_tui_tool_cards.py`, `TorchlightApp`, `LlamaCppClient`, `rlm_engine_optimized.py`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `GitFileTree`, `ToolCallCard`, `test_tui_plan_panel.py`, `on`, `CloudClient`, `CommandPalette`, `MessageCard`, `test_tui_diff_view.py`, `LlamaCppClient`, `PromptTextArea`, `AutonomousHarness`, `TrajectoryRail`, `tui_app.py`, `RLMEngineOptimized`, `test_tui_tool_cards.py`, `CopySelectionModal`?**
