...
## Corpus Check
- 194 files · ~145,297 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2964 nodes · 5278 edges · 212 communities (165 shown, 47 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 255 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f28f359b`
...
1. `TorchlightApp` - 89 edges
2. `TieredMemory` - 83 edges
3. `TieredMemory` - 67 edges
4. `MemoryConfig` - 43 edges
...
Cohesion: 0.08
Nodes (25): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), test_list_available_models_includes_gemma4e4b() (+17 more)
...
Cohesion: 0.05
Nodes (18): App, FileSelected, NodeSelected, copy_to_clipboard(), Codex / Tiny-Brain 2 Style Agent IDE TUI., Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline., Mount a widget defensively and scroll safely after layout pass. (+10 more)
...
Cohesion: 0.09
Nodes (35): _extract_dep_installs(), _extract_failing_tests(), MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle (+27 more)
...
Cohesion: 0.11
Nodes (5): ComposeResult, DirectoryTree, ApprovalModal, Modal dialog for tool & file modification approval in Cyberpunk Sci-Fi style., Snapshot file contents *before* a diffable write executes.          ``_handle_st

### Community 54 - "tui_app.py"
Cohesion: 0.21
Nodes (11): create_client(), load_last_state(), main(), _provider_runtime_info(), Torchlight Agent — Codex / Tiny-Brain 2 Style IDE TUI (Textual) Full-featured ID, Return (port, externally_managed) for a given provider key.      externally_mana, # NOTE: The model badge button click is handled by on_model_badge_clicked, save_last_state() (+3 more)
...
Cohesion: 0.06
Nodes (19): main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., ContextSnapshot, Message, Load persistent project memory (.context-memory.json) into L0 working state., Persist L0 working state to disk in .context-memory.json., Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale. (+11 more)
...
Cohesion: 0.08
Nodes (14): CompressionConfig, _extract_errors(), _extract_file_paths(), ContextSnapshot, Message, MessageRole, Pin a recently-read file so it survives compression.          If the file is alr, Re-read an edited file from disk and update its pin in memory. (+6 more)
...
Cohesion: 0.20
Nodes (7): _beam_budget(), Estimate tokens consumed by system prompt, tools, and flashlight beam., Return (max_beam_files, max_lines_per_file) for the given context size., General conversation., test_format_l0_scratchpad_includes_pending_tasks(), ExecutionFeedbackLoop, TieredMemory
...
Cohesion: 0.08
Nodes (28): _FakeProc, anyio, Tests for Phase-4 git-aware file tree (porcelain parsing + label decoration)., test_git_tree_decorates_file_labels(), test_normalize_status_code(), test_parse_git_status_porcelain_basic(), test_parse_git_status_porcelain_quoted_path(), test_parse_git_status_porcelain_rename_takes_destination() (+20 more)
...
### Community 184 - "test_prompts_and_memory.py"
Cohesion: 0.19
...
Cohesion: 0.22
Nodes (7): diff_summary(), DiffView, ComposeResult, Container, Inline diff rendering for the Torchlight TUI.  Phase 3 of the UI-improvements pl, A unified diff card: header (path + +N −M) + colored body.      Rendered client-, Short "+N −M" stat for a diff entry list ('' when empty).
...
### Community 204 - "core/execution/__init__.py"
Cohesion: 0.50
...
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_app_write_step_renders_diff_card`, `test_tui_plan_panel.py`, `on`, `test_tui_transcript_widgets.py`, `test_tui_diff_view.py`, `GitFileTree`, `ApprovalModal`, `tui_app.py`, `test_tui_trajectory_rail.py`, `AgentStatusModal`, `test_tui_tool_cards.py`, `.__init__`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `LLMStateExtractor`, `TokenCounter`, `_EvictingDeque`, `context_manager/memory/manager.py`, `.clear`, `ProjectMemory`, `build_embedder`?**
