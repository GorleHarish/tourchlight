...
## Corpus Check
- 266 files · ~242,736 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4477 nodes · 9361 edges · 238 communities (210 shown, 28 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 899 edges (avg confidence: 0.55)
...
1. `TorchlightApp` - 215 edges
2. `TieredMemory` - 178 edges
3. `RLMEngineOptimized` - 141 edges
4. `MemoryConfig` - 104 edges
5. `AutonomousHarness` - 69 edges
6. `ExecutionFeedbackLoop` - 56 edges
7. `LlamaCppClient` - 55 edges
8. `tool_edit_file_impl()` - 50 edges
9. `SkillUploadModal` - 49 edges
10. `CloudClient` - 48 edges
...
### Community 2 - "MemoryConfig"
Cohesion: 0.10
Nodes (28): MemoryConfig, Verify assistant conversational answers are never replaced with deduplication pl, Verify Code Mode injects task matrix from implementation_plan.md into L0 working, test_assistant_messages_never_deduplicated(), test_code_mode_scratchpad_task_matrix_injection(), test_l0_scratchpad_priority_weighted_order(), Unit tests for manual context compaction trigger and 85%/91% threshold logic., test_compress_recent_truncates_oversized_tool_output_when_older_empty() (+20 more)
...
Cohesion: 0.08
Nodes (42): Conversation Summarizer for Torchlight.  Extracts key information from conversat, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, ContentChunk, ContentType, ContextSnapshot, ExecutionMode, MemoryEventType (+34 more)

### Community 5 - "task_helpers.py"
Cohesion: 0.17
Nodes (17): test_insert_task_into_plan_section(), Tests for robust task and status tracking in LLM context and TUI., test_compact_task_matrix_adaptive_rendering(), test_l0_scratchpad_integration(), test_status_badges_and_boxes(), test_validate_task_transition(), _clean_task_text(), get_compact_task_matrix() (+9 more)
...
Cohesion: 0.05
Nodes (61): test_set_ctx_window(), calculate_in_memory_diff(), Calculate exact lines added and deleted between two string buffers in RAM., test_detect_stubs(), test_sync_ast_graph_skips_markup(), test_list_dir_impl(), Tool risk classification for Torchlight.  Shared between CLI and TUI frontends., _auto_repair() (+53 more)
...
Cohesion: 0.07
Nodes (47): Unit tests for Torchlight Zero-Context Code Quality Harness., test_check_syntax_js_bracket_balance(), test_check_syntax_js_string_literal_brackets(), test_check_syntax_json(), test_check_syntax_python(), test_compile_gate_rejects_return_outside_function(), test_detect_symptom_patching(), test_edit_file_blocks_broken_syntax() (+39 more)

### Community 12 - "on"
Cohesion: 0.05
Nodes (10): DirectorySelected, FileSelected, Any, on, Pressed, Selected, Submitted, Update the inner static widget content. (+2 more)
...
### Community 14 - "EditorTab"
Cohesion: 0.10
Nodes (15): EditorTab, Message, MouseDown, Clean single-line tab widget displaying file name with close button., Emitted when tab is clicked to switch active file., Emitted when tab close button '×' is clicked., Emitted when tab is right-clicked., TabClosed (+7 more)

### Community 15 - "RLMEngineOptimized"
Cohesion: 0.06
Nodes (39): Verify RLMEngineOptimized._parse_response gracefully handles markdown json tool, Verify RLMEngineOptimized._parse_response parses bracket and CLI tool calls like, Verify RLMEngineOptimized._parse_response parses XML self-closing and attribute, test_engine_parse_bracket_tool_calls(), test_engine_parse_direct_xml_attribute_tool_calls(), test_engine_parse_markdown_json_block_fallback(), Verifies that when a target file already exists on disk, the verification gate, Verifies that _parse_response intercepts code blocks preceded by 'Here is game.j (+31 more)
...
Cohesion: 0.10
Nodes (28): test_ask_user_schema_and_formatting(), test_get_openai_tools_schema(), test_get_schemas_for_phase(), test_registry_get_description_block_phase(), test_validate_tool_call_alias(), test_validate_tool_call_coercion(), test_validate_tool_call_list_dir_optional_path(), test_validate_tool_call_missing_required() (+20 more)

### Community 19 - "ConversationSummarizer"
Cohesion: 0.29
...
Cohesion: 0.09
Nodes (22): Re-export ExecutionFeedbackLoop and TestRunResult from shared core library core., Execution feedback loop for Torchlight., Enum, str, TaskStatus, Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an (+14 more)
...
Cohesion: 0.07
Nodes (45): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_read_symbols_indented_methods_and_duplicate_names(), test_run_command_intercept_ast_functions(), test_search_ast_action_aliases(), test_search_ast_after_writing_file(), test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_verify_compile_param() (+37 more)

### Community 28 - "get_tool_registry"
Cohesion: 0.06
Nodes (37): test_get_tool_registry(), test_tool_registry_execute(), test_tool_registry_execute_unknown(), test_tool_registry_get(), test_tool_registry_preview_dry_run(), test_tool_registry_register(), test_tool_registry_risk_level(), test_tool_registry_risk_level_run_command() (+29 more)

### Community 29 - "test_tui_command_palette.py"
Cohesion: 0.09
Nodes (31): Binding, test_slash_command_list_has_wipe_and_compact(), anyio, Tests for Phase-4 command palette + prompt autocomplete., test_add_context_chip_image_prefix(), test_build_palette_items_kinds_and_visibility(), test_command_palette_composes_filters_and_selects(), test_command_palette_enter_runs_highlighted_item() (+23 more)
...
### Community 31 - "WebOutcomeInspector"
Cohesion: 0.09
...
### Community 32 - "TieredMemory"
Cohesion: 0.05
Nodes (36): ContextSnapshot, Message, MessageRole, Path, Build the message list for the LLM.          Pinned files and dynamic L0 Scratch, Run semantic deduplication on current message history.                  Args:, Enable or disable semantic deduplication., Persist deduplication cache to project memory. (+28 more)
...
Cohesion: 0.08
Nodes (33): ExecutionFeedbackLoop, extract_surgical_traceback(), Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Freshly verify any modified-but-unverified files and return True if         ever, Fetch speculative background test result if running, else execute tests synchron, Convert current failing TestRunResult into a structured TestFailureError for Rec, Build feedback context string for the LLM with surgical error injection. (+25 more)

### Community 38 - "InferenceParams"
Cohesion: 0.06
Nodes (30): detect_model_traits(), InferenceParams, Detect architecture traits (size, reasoning status, vision capability) from mode, One-line description of current params., Convert to API payload dict, excluding None and default values., Writing code files. Near-deterministic — exact syntax matters., Reasoning through plans. Moderate creativity. All tools remain available., Diagnosing errors. Slightly more exploration. (+22 more)
...
Cohesion: 0.06
Nodes (43): NamedTuple, LlamaCppClient, Async implementation of chat protocol method required by LLMClient / DebateVerif, Async streaming implementation required by LLMClient protocol.          Runs the, Step, AgentMemoryWidget, ApprovalModal, CopySelectionModal (+35 more)

### Community 41 - "AgentStatusModal"
Cohesion: 0.13
Nodes (9): AgentStatusModal, Modal dialog for complete visibility into background agent actions & status tele, CenterEmptyState, Container, Pressed, CenterEmptyState — the welcome / idle screen shown in the editor pane.  Replaces, Switch displayed content based on connection state., Route chip buttons to app-level actions. (+1 more)
...
Cohesion: 0.07
Nodes (42): Unit tests for core/tools/parser.py tolerant tool parser & fuzzy repair engine., test_extract_balanced_json_object(), test_parse_tool_call_payload(), test_repair_unclosed_action_tags(), test_repair_unclosed_tool_call_tag(), test_single_quoted_dict_parsing(), test_strip_interleaved_prose(), test_strip_thinking_tags() (+34 more)
...
Cohesion: 0.07
Nodes (20): _clamp(), ContextBudget, Adaptive, headroom-driven context budget coordinator for Torchlight.  Static res, Token reserve kept for the recent-message window., Current fraction of the target window in use., Effective budget allocations for the current turn.      `used_tokens` is the liv, Token allowance for the L0 working memory scratchpad this turn., Max characters per scratchpad entry (longer when headroom is ample). (+12 more)
...
Cohesion: 0.09
Nodes (33): test_format_model_display_names(), test_is_valid_mlx_directory(), test_list_available_draft_models(), test_list_available_models_includes_gemma(), test_normalize_gemma_2_and_3_variants(), test_normalize_gemma_4_4e4b_variants(), test_normalize_gemma_4_e2b_variants(), BenchmarkRecord (+25 more)

### Community 61 - "StreamingChatSession"
Cohesion: 0.13
Nodes (13): chat(), get_phase_system_prompt(), Panel, Build the final message list for the LLM, respecting the context budget., Return a compact Rich markup string showing per-section token estimates., /params                    — show current params         /params auto, Start an interactive chat session with context management and flashlight., Re-append closing tags and unclosed JSON braces that were consumed as stop token (+5 more)
...
### Community 63 - "Static"
Cohesion: 0.16
Nodes (9): DirectoryTree, ComposeResult, VerticalScroll, ComposeResult, ComposeResult, ComposeResult, ComposeResult, Static (+1 more)

### Community 64 - "get_image_metadata"
Cohesion: 0.16
Nodes (21): Any, Convert Message to provider-compatible API payload dictionary., test_get_image_mime_type(), build_image_data_url(), encode_image_to_base64(), format_image_text_summary(), format_ollama_vision_payload(), format_openai_vision_content() (+13 more)
...
### Community 67 - "test_tui_file_tree.py"
Cohesion: 0.06
...
Cohesion: 0.06
Nodes (34): anyio, Tests for Phase-1 transcript widgets (message cards, streaming, thinking)., Smoke test: the real app mounts MessageCards and drives the streaming view., Verify MessageCard copy and reuse actions work for user and assistant messages., Verify MessageCard duration formatting, empty user headers, and timestamp overri, test_app_transcript_wiring(), test_card_meta_for(), test_estimate_token_count() (+26 more)

### Community 69 - "core.py"
Cohesion: 0.09
Nodes (28): CoreToolRegistry, get_core_registry(), Any, Core Tools — built-in, always available, called via TOOL_NAME("arg") syntax.  Th, Compatibility subclass of ToolRegistry providing CLI-specific execute/dangerous_, test_classify_destructive_command(), test_classify_empty_command(), test_classify_install_command() (+20 more)

### Community 70 - "LLMStateExtractor"
Cohesion: 0.15
...
### Community 77 - "test_autonomous_harness.py"
Cohesion: 0.28
...
### Community 79 - "cloud_client.py"
Cohesion: 0.13
Nodes (8): _HttpxOpenAIChatCompletions, _HttpxOpenAIClient, _HttpxOpenAIModels, Return the ids of models the provider currently reports as available.         Us, Resolve requested self.model against live models to prevent 404 mismatches., Sanitize message roles. Convert system role to user role for models (e.g. Gemma), Async streaming implementation required by LLMClient protocol., _sanitize_messages_for_cloud()
...
### Community 82 - ".update_status_bar"
Cohesion: 0.11
Nodes (12): is_port_in_use(), Check if server port 8080 is actively listening., _provider_runtime_info(), Return (port, externally_managed) for a given provider key.      externally_mana, Keep input spinner clean and minimal while directing elapsed time         and li, Sync all connection-dependent UI elements.          Called on server status chan, Populate the model dropdown near chat toolbar based strictly on selected backend, Unified model loader: configures provider, client, port runtime, and starts back (+4 more)

### Community 83 - "CloudClient"
Cohesion: 0.09
Nodes (23): test_rlm_engine_solve_method(), CloudClient, create_client(), display_step(), get_depth_style(), main(), create_client(), print_banner() (+15 more)
...
### Community 89 - "fetch_provider_models"
Cohesion: 0.17
Nodes (6): fetch_provider_models(), Query an OpenAI-compatible /models endpoint (LM Studio, Ollama, llama.cpp)     o, Changed, Get model choices tailored to the selected inference backend., Get model choices strictly tailored ONLY to the selected inference backend., Return all available models across MLX, TurboQuant GGUF, LM Studio, and Ollama.
...
### Community 95 - "test_tui_tabbed_editor.py"
Cohesion: 0.12
...
### Community 113 - "._handle_slash_command"
Cohesion: 0.17
Nodes (6): copy_to_clipboard(), load_last_state(), main(), Copy text to system clipboard across macOS, Linux, and Windows., Open the Inference Engine & TurboQuant configuration modal., save_last_state()
...
Cohesion: 0.15
Nodes (20): _build_excerpt(), LLM-powered SessionState extractor.  Replaces the regex-based _merge_summary_int, Build a compact conversation view for the extraction prompt., ContentChunk, ContentType, ContextSnapshot, Message, MessageRole (+12 more)

### Community 120 - "ToolCallCard"
Cohesion: 0.06
...
Cohesion: 0.08
Nodes (27): anyio, asyncio, Unit and integration tests for Torchlight Plan Mode (ExecutionMode.PLAN)., Verify StreamingChatSession in Plan Mode correctly handles plan creation and ver, Verify ExecutionMode.PLAN can be set on SessionState., Verify that when a model outputs markdown plan with pseudocode $ WRITE_FILE, it, Verify that in Plan Mode, tool executions targeting non-plan files (e.g. index.h, Verify that ASK_USER delegates to engine.ask_user_fn when registered. (+19 more)
...
### Community 135 - "test_multimodal_vision.py"
Cohesion: 0.12
Nodes (26): fixture, Path, Unit and integration tests for multimodal vision capabilities (Gemma 3, Qwen VL,, Verify Message.to_dict generates rich text description when vision_supported is, Verify format_image_text_summary extracts metadata and embeds exact calling cont, Verify validate_tool_call auto-heals missing/misplaced path arguments for VIEW_I, Create a minimal 1x1 valid PNG image., Verify TieredMemory records relative active image paths and surfaces them in L0 (+18 more)
...
### Community 144 - "test_prompts_and_memory.py"
Cohesion: 0.07
Nodes (35): extract_modified_symbols(), is_valid_file_path(), Validate if a string is a genuine file path rather than code attribute access (e, Extract function/class AST symbol names modified or added between old and new te, get_phase_system_prompt(), Remove raw tool payload dumps (Params:, Result:, Writing code to file: ...) and, Generate phase-tailored system prompt by selecting the dedicated phase template, sanitize_assistant_text() (+27 more)

### Community 145 - "SkillResult"
Cohesion: 0.11
Nodes (12): MyCustomSkill, Any, A template for creating your own custom tools for the agent.     Place your logi, Any, ReproSkill, MarkdownDocumentSkill, Any, Lightweight skill backed by SKILL.md.      This lets users add modular markdown (+4 more)
...
Cohesion: 0.06
Nodes (33): End-to-end verification tests for preventing repeated edit loops, out-of-bounds, Verifies that calling WRITE_FILE with a tiny snippet on an established file, Verifies that calling EDIT_FILE with start_line beyond file length     does NOT, Verifies that 'L106' and start_line without end_line are safely parsed and bound, Verifies that a valid in-bounds single line edit (e.g. line 2) without old_text, Verifies TrajectoryLock catches sequential range stepping when line numbers use, Verifies that target file path aliases (target, file, filename, file_path)     a, Verifies that real code files containing HTML placeholders or TODO comments (+25 more)
...
### Community 162 - ".update_sidebar_meta"
Cohesion: 0.10
Nodes (9): Infer the current agent phase from user input and the last model response., Step, Return the current project root path from engine or working directory., Mount a widget defensively and scroll safely after layout pass., Mount a running ToolCallCard for a streamed ``<tool_call>`` marker.          Kep, Manually trigger memory context compaction., build_plan_overview_text(), Render Implementation Plan overview (title & mode badge). (+1 more)
...
Cohesion: 0.09
Nodes (34): Verify TUI format helpers render CODE MODE badge with task checklist., test_code_mode_badge_in_tui_format(), test_is_task_match_precision(), test_mark_task_status_by_number(), test_parse_numbered_tasks_from_markdown(), test_phase_wise_tasks_and_hierarchical_marking(), test_status_summary_counts_verified_and_completed(), test_add_subtask_survives_sync_and_lands_in_plan() (+26 more)

### Community 189 - "PaneResizer"
Cohesion: 0.33
...
### Community 196 - "open_file_in_system_app"
Cohesion: 0.33
Nodes (4): open_file_in_system_app(), on, Pressed, Open a file with the operating system default application.

### Community 198 - "core/api/lmstudio.py"
Cohesion: 0.40
...
### Community 200 - ".solve_async"
Cohesion: 0.25
Nodes (5): Return True if the most recent test run actually ran and has failing or, Check if active LLM client or server supports multimodal image inputs., Build an explicit warning attached to an accepted final answer when the, Validate tool call against schema and normalize parameter aliases.          Retu, validate_and_normalize_tool_call()

### Community 201 - "dashboard.py"
Cohesion: 0.33
...
### Community 202 - "._submit_user_input"
Cohesion: 0.12
Nodes (6): Extract text from the TextArea, clear it, and dispatch., Programmatic send — called by ctrl+enter binding., Global key bindings that aren't caught by specific widgets.          Key contrac, Focus and open the model select dropdown (model badge click / toolbar)., Posted when the user presses Enter with no active suggestion., SubmitRequested

### Community 203 - "Flashlight"
Cohesion: 0.26
Nodes (4): _beam_config_for_context(), Flashlight, FileEntry, SymbolIndex

### Community 204 - "generate_ansi_image_preview"
Cohesion: 0.40
Nodes (3): generate_ansi_image_preview(), Generate a Rich Text object containing a 24-bit ANSI half-block color preview of, ImageViewer and BinaryFileViewer widgets for the Torchlight TUI.  Provides rich,
...
Cohesion: 0.13
Nodes (14): test_tool_registry_and_schemas_view_image(), classify_tool(), Classify a tool call into AUTO, CONFIRM, or REVIEW risk tier., escape_markup(), extract_code_blocks(), Rich transcript widgets for the Torchlight TUI.  Phase 1 of the UI-improvements, Truncate tool output for the UI: hard char + line caps., Current time as a compact HH:MM label (local time, display only). (+6 more)
...
### Community 225 - ".action_wipe_session"
Cohesion: 0.20
Nodes (3): Realtime task-state updates surfaced to the Output log., Append a line to the Output tab's RichLog.          severity: 'info' | 'tool' |, Completely wipe session context and memory, starting a clean fresh session.

### Community 227 - "test_tui_trajectory_rail.py"
Cohesion: 0.24
...
Cohesion: 0.18
Nodes (11): command, compress_file(), count_tokens(), goal(), plan(), Start a planning session to brainstorm and write/update implementation_plan.md., Start an autonomous goal execution session driven by .torchlight task tracking., Compress a file using verbatim compaction. (+3 more)

### Community 236 - "is_image_file"
Cohesion: 0.29
Nodes (7): test_is_image_file(), Inspect an image file, validate its structure, and attach to active memory., view_image(), extract_image_paths_from_text(), is_image_file(), Scan user prompt for image file paths or markdown image tags:     - Markdown: ![, Check if the given path or filename has an image extension.

### Community 237 - "mark_task_in_progress"
Cohesion: 0.50
Nodes (4): Verify that marking a task in_progress maintains single-active exclusivity., test_single_in_progress_task_exclusivity(), mark_task_in_progress(), Mark a task as in_progress (started).
...
## Knowledge Gaps
- **457 isolated node(s):** `$schema`, `.opencode/plugins/graphify.js`, `context-manager-cli`, `run.sh script`, `COLORTERM` (+452 more)
...
- **Why does `RLMEngineOptimized` connect `RLMEngineOptimized` to `TorchlightApp`, `MemoryConfig`, `test_plan_mode.py`, `cli/main.py`, `context_manager/compression/summarizer.py`, `EditorTab`, `test_prompts_and_memory.py`, `test_repeated_edit_loop_fix.py`, `get_tool_registry`, `test_tui_diff_view.py`, `TieredMemory`, `.update_sidebar_meta`, `DebateVerifier`, `ExecutionFeedbackLoop`, `InferenceParams`, `tui_app.py`, `AgentStatusModal`, `tool_edit_file_impl`, `test_tool_parser.py`, `test_tui_image_viewer.py`, `ProjectMemory`, `TrajectoryLock`, `PaneResizer`, `test_goal_mode_process.py`, `MessageCard`, `.solve_async`, `._stream_llm_with_retry`, `anyio`, `._update_params`, `test_code_mode_phase_detection_persistence`, `main_optimized.py`, `test_tui_plan_panel.py`, `test_plan_execution_loop.py`, `test_engine_code_mode_setting`, `test_tui_context_breakdown.py`, `test_tui_tabbed_editor.py`, `ProjectMemory`, `test_tui_trajectory_rail.py`, `test_resizer.py`, `AskUserModal`, `._estimate_l0_tokens`, `SkillUploadModal`, `REPLSandbox`, `._handle_slash_command`, `datetime`, `ToolCallCard`, `.set_execution_mode_callback`, `test_ring_buffer_prompt_dedup_skip`?**
  _High betweenness centrality (0.143) - this node is a cross-community bridge._
- **Why does `TorchlightApp` connect `TorchlightApp` to `MemoryConfig`, `on`, `._apply_pane_widths`, `EditorTab`, `RLMEngineOptimized`, `test_tui_diff_view.py`, `.update_sidebar_meta`, `tui_app.py`, `AgentStatusModal`, `test_tui_accessibility.py`, `test_tui_image_viewer.py`, `test_model_normalization.py`, `Static`, `ConnectionPill`, `MessageCard`, `CommandPalette`, `._submit_user_input`, `test_tui_plan_panel.py`, `.update_status_bar`, `CloudClient`, `test_tui_context_breakdown.py`, `TranscriptView`, `fetch_provider_models`, `AutonomousHarness`, `TrajectoryRail`, `test_tui_tabbed_editor.py`, `.action_wipe_session`, `test_tui_theme.py`, `test_tui_trajectory_rail.py`, `test_resizer.py`, `._handle_slash_command`, `ToolCallCard`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `MemoryConfig`, `cli/main.py`, `task_helpers.py`, `context_manager/compression/summarizer.py`, `test_multimodal_vision.py`, `MemoryObject`, `RLMEngineOptimized`, `test_prompts_and_memory.py`, `DeduplicationEngine`, `TestRunResult`, `config.py`, `TokenCounter`, `get_tool_registry`, `.update_sidebar_meta`, `tui_app.py`, `tool_edit_file_impl`, `ContextBudget`, `test_scratchpad_ui_format.py`, `StreamingChatSession`, `.solve_async`, `test_autonomous_harness.py`, `.__init__`, `test_plan_execution_loop.py`, `CloudClient`, `.__init__`, `test_tui_context_breakdown.py`, `AutonomousHarness`, `command`, `._estimate_l0_tokens`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Are the 25 inferred relationships involving `TorchlightApp` (e.g. with `DummyClient` and `_StubClient`) actually correct?**
  _`TorchlightApp` has 25 INFERRED edges - model-reasoned connections that need verification._
- **Are the 30 inferred relationships involving `TieredMemory` (e.g. with `StreamingChatSession` and `AutonomousHarness`) actually correct?**
  _`TieredMemory` has 30 INFERRED edges - model-reasoned connections that need verification._
- **Are the 34 inferred relationships involving `RLMEngineOptimized` (e.g. with `DummyClient` and `ConversationSummarizer`) actually correct?**
  _`RLMEngineOptimized` has 34 INFERRED edges - model-reasoned connections that need verification._
