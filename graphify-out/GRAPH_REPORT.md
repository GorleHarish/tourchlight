...
## Corpus Check
- 176 files · ~129,593 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2604 nodes · 4713 edges · 178 communities (141 shown, 37 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 236 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `0ddfbd45`
...
Cohesion: 0.11
Nodes (24): test_tool_context_window_scaling(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_file_impl() (+16 more)
...
### Community 38 - "on"
Cohesion: 0.13
Nodes (7): DirectorySelected, on, FileActionModal, FolderPickerModal, Modal dialog for interactive visual folder selection across the entire computer., Modal dialog presenting OS options when a file is selected in the Explorer tree., Submitted
...
### Community 40 - "._handle_slash_command"
Cohesion: 0.17
Nodes (5): copy_to_clipboard(), Build the AST knowledge graph silently in background thread., Manually trigger memory context compaction., Copy text to system clipboard across macOS, Linux, and Windows., work
...
Cohesion: 0.12
Nodes (30): _build_excerpt(), LLM-powered SessionState extractor.  Replaces the regex-based _merge_summary_int, Build a compact conversation view for the extraction prompt., MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot (+22 more)
...
### Community 46 - "rlm_engine_optimized.py"
Cohesion: 0.19
Nodes (10): get_token_counter(), Token counting for Torchlight.  Uses tiktoken when available, falls back to a wo, TokenCounter, test_get_token_counter_caching(), test_get_token_counter_different_models(), test_token_counter_basic(), test_token_counter_empty(), test_token_counter_truncate_long() (+2 more)
...
### Community 49 - "Static"
Cohesion: 0.16
Nodes (4): ComposeResult, Mount a widget defensively and scroll safely after layout pass., Static, Step
...
Cohesion: 0.10
Nodes (4): ApprovalModal, Modal dialog for tool & file modification approval., Modal dialog for selecting session execution mode (Chat vs Goal)., SessionModePickerModal
...
Cohesion: 0.14
Nodes (7): LlamaCppClient, Ensure strict role alternation (user, assistant...) and merge consecutive same-r, Async implementation of chat protocol method required by LLMClient / DebateVerif, Async streaming implementation required by LLMClient protocol., _sanitize_messages(), RLMEngine, Step
...
### Community 108 - "ModelPickerModal"
Cohesion: 0.25
...
Cohesion: 0.06
Nodes (42): anyio, test_verification_gate_allows_final_answer_when_all_done(), test_verification_gate_rejects_premature_final_answer(), test_rlm_engine_code_tag_and_backticks(), test_rlm_engine_debate_verifier_error_resilience(), test_rlm_engine_optimized_code_execution(), test_rlm_engine_optimized_debate_verifier_initialization(), test_rlm_engine_optimized_none_tool_name() (+34 more)

### Community 119 - "get_tool_registry"
Cohesion: 0.33
Nodes (6): test_batch_tool_execution(), test_get_tool_registry(), test_tool_registry_preview_dry_run(), test_save_memory_tool(), get_tool_registry(), Get or create the global tool registry.
...
Cohesion: 0.32
Nodes (5): Ensure target project has local git repository and persistent memory initialized, test_format_l0_scratchpad_includes_pending_tasks(), ExecutionFeedbackLoop, Path, TieredMemory
...
Cohesion: 0.33
Nodes (6): Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, TaskStatus, Tell the tool layer what context window the current model has., set_ctx_window(), Enum, str

### Community 177 - "TorchlightApp"
Cohesion: 0.09
Nodes (7): App, FileSelected, NodeSelected, Extract text from the TextArea, clear it, and dispatch., Enter submits the prompt; Shift+Enter inserts a newline., Codex / Tiny-Brain 2 Style Agent IDE TUI., TorchlightApp

## Knowledge Gaps
- **347 isolated node(s):** `COLORTERM`, `TERM`, `PYTHONPATH`, `Summary of Tracked Bugs`, `BUG-01: `_total_llm_calls` Counter Accumulation Across Sessions` (+342 more)
...
- **Why does `TorchlightApp` connect `TorchlightApp` to `test_tui_plan_panel.py`, `on`, `._handle_slash_command`, `AgentStatusModal`, `ModelPickerModal`, `Static`, `tui_app.py`, `RLMEngineOptimized`, `ApprovalModal`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `BaseSkill`, `HTMLGameSkill`, `TDDSkill`, `MarkdownDocumentSkill`, `CalculatorSkill`, `PlanningSkill`, `UnifiedSkillRegistry`, `TDDSkill`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `LLMStateExtractor`, `LlamaCppClient`, `TokenCounter`, `_EvictingDeque`, `context_manager/memory/manager.py`, `ProjectMemory`, `.clear`, `build_embedder`?**
...
- **What connects `COLORTERM`, `TERM`, `PYTHONPATH` to the rest of the system?**
