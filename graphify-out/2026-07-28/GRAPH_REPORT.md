...
## Corpus Check
- 159 files · ~244,081 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2368 nodes · 4219 edges · 162 communities (139 shown, 23 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 209 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e13793ff`
...
- `SolveResult` --uses--> `ExecutionFeedbackLoop`  [INFERRED]
  rlm_optimized/rlm_engine_optimized.py → core/execution/feedback_loop.py
- `Step` --uses--> `ExecutionFeedbackLoop`  [INFERRED]
  rlm_optimized/rlm_engine_optimized.py → core/execution/feedback_loop.py
- `RLMEngine` --uses--> `MemoryConfig`  [INFERRED]
  rlm_optimized/rlm_engine.py → context-manager-cli/src/context_manager/memory/manager.py
...
Cohesion: 0.17
Nodes (15): Recovery engine for Torchlight errors.  Provides structured recovery strategies, Tracks retry state for a specific error pattern., RecoveryState, ContextOverflowError, ParseError, Enum, Structured error types for Torchlight.  Every error carries context for automate, Tool call failed schema validation. (+7 more)
...
Cohesion: 0.12
Nodes (29): _diagnose(), ProjectSnapshot, Path, AndroidTroubleshootSkill — auto-loaded by Torchlight at startup.  Diagnoses and, True if ANY of the given signals are present., True if pattern found in any of the named file labels., Prefix a step string with its status tag., Lightweight scan of an Android project directory.      Reads (if present): (+21 more)
...
Cohesion: 0.08
Nodes (23): ABC, BaseSkill, CalculatorSkill, create_default_registry(), _extract_markdown_skill_metadata(), GitSkill, _LazySkill, MarkdownDocumentSkill (+15 more)
...
### Community 14 - "StaticHTMLValidator"
Cohesion: 0.09
Nodes (19): EphemeralHTTPServer, Any, Path, QuietHTTPRequestHandler, Web Outcome Inspector for Torchlight.  Provides low-memory, ephemeral runtime an, Tier 1: Static HTML syntax and asset path validator., Main Inspector Subsystem driving zero-memory, ephemeral web verification., Tier 3: Run Node JSDOM script if node is available. (+11 more)
...
### Community 21 - "Step"
Cohesion: 0.29
Nodes (6): ConversationSummarizer, Summarizer with LLM-powered and rule-based fallback paths.      When an llm_clie, MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, SolveResult, Step
...
Cohesion: 0.05
Nodes (33): ActionTracker, command, verify_cli_prompt(), _beam_budget(), chat(), compress_file(), count_tokens(), Compress a file using verbatim compaction. (+25 more)
...
### Community 34 - "context_manager/memory/persistence.py"
Cohesion: 0.22
...
### Community 43 - "context_manager/memory/models.py"
Cohesion: 0.15
Nodes (15): _build_excerpt(), _merge_into_state(), _parse_json_response(), LLM-powered SessionState extractor.  Replaces the regex-based _merge_summary_int, Robustly extract a JSON object from the model's response.      Local models some, Merge the extracted JSON fields into the existing SessionState.      Strategy: L, Run LLM extraction and merge findings into *state* in-place.          Returns Tr, Build a compact conversation view for the extraction prompt. (+7 more)
...
### Community 50 - "context-manager-cli/tests/test_models.py"
Cohesion: 0.18
Nodes (13): ContentChunk, MemoryNeedle, MemoryObject, Message, SessionState, test_content_chunk_custom(), test_content_chunk_defaults(), test_memory_needle_custom() (+5 more)
...
Cohesion: 0.12
Nodes (14): test_rlm_engine_debate_verifier_error_resilience(), test_rlm_engine_optimized_code_execution(), test_rlm_engine_optimized_debate_verifier_initialization(), test_rlm_engine_optimized_none_tool_name(), Path, _clean_and_parse_json(), Switch the active workspace, keeping the sandbox's AST-graph         lookups (ge, Notify listeners of real-time background status and action telemetry. (+6 more)
...
### Community 65 - "rlm_engine.py"
Cohesion: 0.08
Nodes (23): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), _detect_apple_silicon_ram() (+15 more)
...
Cohesion: 0.14
Nodes (16): test_search_ast_schema_validation(), test_write_file_impl(), test_write_file_impl_missing_path(), Tests for performance and accuracy optimizations in Torchlight., test_batch_tool_execution(), test_inline_syntax_guardrail(), test_symbol_index_mtime_cache(), test_get_tool_registry() (+8 more)
...
### Community 108 - "test_errors.py"
Cohesion: 0.15
Nodes (11): ConnectionError, LLM backend unreachable., Path or command outside allowed scope., SecurityError, test_connection_error(), test_parse_error(), test_security_error_not_recoverable(), test_tool_error() (+3 more)
...
Cohesion: 0.13
Nodes (18): Post-edit auto-run test suite failure., TestFailureError, ExecutionFeedbackLoop, extract_surgical_traceback(), Enum, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run. (+10 more)
...
### Community 152 - "TorchlightError"
Cohesion: 0.22
Nodes (7): Generate a dedup key for this error type., Decide what to do after an error.          Returns a RecoveryAction indicating t, Check if we should escalate to the user after exhausting retries., Base error with structured context., TorchlightError, test_torchlight_error_with_context(), Exception
...
### Community 155 - "autonomous_harness.py"
Cohesion: 0.39
Nodes (5): GoalSpec, Enum, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, TaskStatus, str

### Community 156 - "LLMStateExtractor"
Cohesion: 0.29
...
### Community 157 - "AndroidTroubleshootSkill"
Cohesion: 0.40
Nodes (3): AndroidTroubleshootSkill, Any, Project-aware Android development error diagnosis.      Scans build.gradle, prog
...
- **Why does `TieredMemory` connect `TieredMemory` to `_EvictingDeque`, `AutonomousHarness`, `RLMEngine`, `autonomous_harness.py`, `._prune_old_messages`, `test_autonomous_harness.py`, `Step`, `context_manager/memory/manager.py`, `deque`, `.append`, `cli/main.py`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `BaseSkill`, `TDDSkill`, `MyCustomSkill`, `unified.py`, `PlanningSkill`, `discovery.py`, `AndroidTroubleshootSkill`, `TDDSkill`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `StreamingChatSession` connect `cli/main.py` to `test_phase_detection.py`, `Step`, `TieredMemory`?**
...
- **Should `TokenCounter` be split into smaller, more focused modules?**
