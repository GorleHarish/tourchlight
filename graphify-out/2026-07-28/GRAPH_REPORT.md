...
## Summary
- 1885 nodes · 3836 edges · 111 communities (91 shown, 20 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 276 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ad0ff4b6`
...
1. `TieredMemory` - 69 edges
2. `TieredMemory` - 59 edges
3. `TorchlightApp` - 48 edges
4. `StreamingChatSession` - 35 edges
5. `AutonomousHarness` - 34 edges
6. `LlamaCppClient` - 32 edges
7. `ProjectSnapshot` - 31 edges
8. `SkillResult` - 31 edges
9. `get_core_registry()` - 30 edges
10. `OllamaClient` - 30 edges
...
Cohesion: 0.08
Nodes (26): test_get_tool_registry(), test_tool_registry_execute(), test_tool_registry_execute_unknown(), test_tool_registry_get(), test_tool_registry_register(), test_tool_registry_risk_level(), test_tool_registry_risk_level_run_command(), test_tool_result_failure() (+18 more)
...
Cohesion: 0.07
Nodes (16): ContextSnapshot, Message, MessageRole, Get the token breakdown bucket for a role., Add tokens to the token breakdown., Remove tokens from the token breakdown., Reset token breakdown to zero., Remove oldest non-system messages to stay under max_messages limit. (+8 more)
...
Cohesion: 0.09
Nodes (32): test_edit_file_diff_block_in_old_text(), test_edit_file_with_diff_block(), test_edit_file_impl(), test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match() (+24 more)
...
Cohesion: 0.09
Nodes (12): ContextSnapshot, Pin a recently-read file slice so it survives compression without bloating conte, Return list of (path, content) for pinned files., Remove all pinned files., Compress older messages, preserving the first N messages., Async wrapper for compress_recent., Build the message list for the LLM.          Pinned files are injected as a syst, Build critical context block from session state. (+4 more)
...
Cohesion: 0.14
Nodes (21): test_search_ast_impl_fallback(), Query Kùzu AST graph (semantic search, signature, source, structure, subgraph)., tool_search_ast_impl(), _ast_db_missing_message(), get_class_signature(), _get_encoder(), get_function_ast(), get_function_source() (+13 more)

### Community 20 - "ExecutionFeedbackLoop"
Cohesion: 0.13
Nodes (11): ExecutionFeedbackLoop, Enum, Path, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Build feedback context string for the LLM., Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Detect and run the project's test suite or web inspector. (+3 more)

### Community 21 - "core/memory/__init__.py"
Cohesion: 0.10
Nodes (30): ConversationSummarizer, Message, Conversation Summarizer for Torchlight.  Extracts key information from conversat, Summarize conversation turns for compression., Create a simple summary of messages., Extract key information from text., _role_label(), ContentChunk (+22 more)

### Community 22 - "core.py"
Cohesion: 0.06
Nodes (46): classify_command(), CoreTool, CoreToolRegistry, _ddg_search(), _detect_doc_source(), _extract_identifiers(), get_core_registry(), Core Tools — built-in, always available, called via TOOL_NAME("arg") syntax.  Ri (+38 more)
...
Cohesion: 0.13
Nodes (16): MemoryConfig, create_mock_feedback_loop(), ExecutionFeedbackLoop, Path, Unit tests for Inter-Task Context Pipeline, Dependencies, and File Collision Gua, test_inter_task_output_summary_injection(), test_target_file_collision_detection(), test_task_dependencies_and_execution_ordering() (+8 more)

### Community 32 - "get_tool_registry"
Cohesion: 0.12
Nodes (20): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_search_ast_schema_validation(), test_batch_tool_execution(), test_get_openai_tools_schema(), test_validate_tool_call_alias(), test_validate_tool_call_missing_required(), test_validate_tool_call_unknown_tool(), test_validate_tool_call_valid() (+12 more)
...
Cohesion: 0.10
Nodes (32): MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, ContentChunk, ContentType, ContextSnapshot, MemoryNeedle, MemoryObject, Message (+24 more)
...
Cohesion: 0.20
Nodes (5): _extract_dep_installs(), _extract_errors(), _extract_failing_tests(), _extract_file_paths(), Pin a recently-read file so it survives compression.          If the file is alr

### Community 40 - "_EvictingDeque"
Cohesion: 0.18
Nodes (8): _EvictingDeque, TokenCounter, Deque that fires a callback when an item is evicted due to maxlen., Validate all tracked file paths against the actual filesystem.         Prunes no, Minimum NEW tokens that must arrive before re-compression is allowed.          S, create_progressive_compressor(), Create a compressor tuned for the given context window.      Always pass the sha, deque
...
### Community 54 - "._execute_tool_with_approval"
Cohesion: 0.40
...
### Community 76 - "cli/main.py"
Cohesion: 0.19
Nodes (8): CompressionConfig, VerbatimCompactor — compress text while preserving code structure., HarnessConfig, Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, datetime
...
### Community 84 - "TaskStatus"
Cohesion: 0.67
Nodes (3): TaskStatus, Enum, str

## Knowledge Gaps
- **58 isolated node(s):** `Ephemeral Web Outcome Inspection Subsystem (`WebOutcomeInspector` & `INSPECT_WEB`)`, `Performance & Accuracy Optimization Suite`, `Inter-Task Context Pipeline & File Collision Guard (`AutonomousHarness`)`, `Strict Context Budget Scaling & Pinned File Accounting`, `Automatic Project & Persistent Memory File Initialization` (+53 more)
...
- **Why does `SkillResult` connect `SkillResult` to `ProjectSnapshot`, `CalculatorSkill`, `BaseSkill`, `TDDSkill`, `UnifiedSkillRegistry`, `MarkdownDocumentSkill`, `PlanningSkill`, `discovery.py`, `TDDSkill`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `RLMEngine`, `AutonomousHarness`, `.__init__`, `cli/main.py`, `test_autonomous_harness.py`, `SelectiveCompressor`, `TaskStatus`, `core/memory/__init__.py`, `RLMEngineOptimized`, `test_diff_edit.py`, `rlm_engine_optimized.py`?**
