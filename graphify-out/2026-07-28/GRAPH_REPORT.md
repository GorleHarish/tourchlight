...
## Corpus Check
- 161 files · ~244,887 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2381 nodes · 4550 edges · 153 communities (134 shown, 19 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 344 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3e7d940c`
...
Cohesion: 0.06
Nodes (23): main(), CLI entry point to launch the Torchlight 24-Hour Autonomous Harness., MemoryConfig, ContextSnapshot, Message, Pin a recently-read file slice so it survives compression without bloating conte, Remove a file from pinned memory if deleted or stale., Re-read an edited file from disk and update its pin in memory. (+15 more)
...
### Community 38 - "on"
Cohesion: 0.07
Nodes (13): ComposeResult, DirectorySelected, on, Pressed, AgentStatusModal, ApprovalModal, CopySelectionModal, FolderPickerModal (+5 more)
...
Cohesion: 0.13
Nodes (16): Autonomous Harness Driver for Torchlight.  Enables continuous, multi-epoch execu, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, get_token_counter(), Token counting for Torchlight.  Uses tiktoken when available, falls back to a wo, TokenCounter, Unit tests for context budget overflow detection and fixes in TieredMemory, RLME, test_tiered_memory_total_tokens_includes_pinned_files(), test_tool_context_window_scaling() (+8 more)
...
### Community 110 - "core/execution/feedback_loop.py"
Cohesion: 0.33
Nodes (8): extract_surgical_traceback(), Enum, Execution Feedback Loop for Torchlight.  Closes the loop between code changes an, Extract strictly surgical failure traceback from test output, removing passing t, TestResultStatus, test_extract_surgical_traceback_ansi_stripping(), test_extract_surgical_traceback_pytest(), test_extract_surgical_traceback_syntax_error()
...
Cohesion: 0.10
Nodes (14): _beam_config_for_context(), BeamResult, Flashlight, FileEntry, SymbolIndex, Flashlight Beam — query-to-code relevance scorer.  Scoring strategy (additive):, FileEntry, Path (+6 more)
...
Cohesion: 0.20
Nodes (9): ExecutionFeedbackLoop, Path, Auto-run tests and web outcome inspection after code changes and inject feedback, Called after a tool is executed. Returns test results if tests were run., Run fast pre-flight auto-fixer/linter on modified files before test execution., Detect and run the project's test suite or web inspector., TestResult, test_feedback_loop_trigger_and_context() (+1 more)
...
Cohesion: 0.24
Nodes (9): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_get_tool_registry(), test_tool_registry_preview_dry_run(), Query AST Knowledge Graph (search, path, subgraph, structure, update, summary)., tool_search_ast_impl(), get_tool_registry() (+1 more)
...
- **Why does `TieredMemory` connect `TieredMemory` to `TokenCounter`, `._update_state_from_message`, `_EvictingDeque`, `AutonomousHarness`, `LLMStateExtractor`, `context_manager/memory/manager.py`, `build_embedder`, `cli/main.py`, `.clear`, `ProjectMemory`, `test_autonomous_harness.py`, `StreamingChatSession`, `.__init__`, `OllamaClient`, `RLMEngineOptimized`?**
  _High betweenness centrality (0.059) - this node is a cross-community bridge._
- **Why does `LMStudioClient` connect `LMStudioClient` to `_HttpxLMStudioClient`, `LLMStateExtractor`, `context_manager/memory/manager.py`, `core.py`, `cli/main.py`, `CoreToolRegistry`, `StreamingChatSession`, `.__init__`, `LLMClient`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `TieredMemory` connect `TieredMemory` to `.solve_async`, `test_autonomous_harness_pipeline.py`, `SelectiveCompressor`, `cli/main.py`, `test_autonomous_harness.py`, `rlm_engine_optimized.py`, `tool_edit_file_impl`, `OllamaClient`?**
