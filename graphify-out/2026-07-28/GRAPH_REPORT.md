...
## Corpus Check
- 158 files · ~243,212 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2354 nodes · 4183 edges · 160 communities (136 shown, 24 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 207 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5baa8cb5`
...
10. `StreamingChatSession` - 25 edges
...
Cohesion: 0.10
Nodes (34): test_list_dir_impl(), _ddg_search(), _detect_doc_source(), _extract_identifiers(), _git_run(), Unified tool implementations for Torchlight.  All tool functions follow the sign, WEB_SEARCH — general web search., WEB_FETCH — fetch and return readable content of a URL. (+26 more)
...
### Community 12 - "core/compression/summarizer.py"
Cohesion: 0.24
...
Cohesion: 0.12
Nodes (24): test_edit_file_impl_not_found(), test_grep_hyphen_pattern(), test_grep_impl(), test_grep_impl_file_path(), test_grep_impl_no_match(), test_read_symbols_impl(), test_run_command_impl(), test_run_command_impl_fail() (+16 more)
...
Cohesion: 0.24
Nodes (8): ConversationSummarizer, Summarizer with LLM-powered and rule-based fallback paths.      When an llm_clie, MemoryConfig, Create a MemoryConfig automatically tuned for the given context window size and, Message, test_message_defaults(), SolveResult, Step
...
Cohesion: 0.10
Nodes (24): Tests for SEARCH_AST tool implementation and Kuzu connection handling., test_search_ast_impl_fallback(), test_search_ast_schema_validation(), test_get_tool_registry(), test_get_openai_tools_schema(), test_validate_tool_call_alias(), test_validate_tool_call_missing_required(), test_validate_tool_call_unknown_tool() (+16 more)
...
Cohesion: 0.09
Nodes (23): MemoryConfig, Tiered Memory Manager for Torchlight.  L0-L3 memory hierarchy with progressive c, Tests for read/edit tool memory synchronization (unpin_file, refresh_pin)., test_memory_refresh_pin(), test_memory_refresh_pin_deleted_file(), test_memory_unpin_file(), test_tiered_memory_pinned_files_without_system_message(), _detect_apple_silicon_ram() (+15 more)
...
### Community 153 - "deque"
Cohesion: 0.22
Nodes (5): Pin a recently-read file so it survives compression.          If the file is alr, Remove a file from pinned memory if deleted or stale., Re-read an edited file from disk and update its pin in memory., TokenCounter, deque
...
- **Why does `TieredMemory` connect `TieredMemory` to `_EvictingDeque`, `AutonomousHarness`, `RLMEngine`, `._prune_old_messages`, `RLMEngineOptimized`, `Message`, `Message`, `cli/main.py`, `deque`, `.append`, `StreamingChatSession`, `.__init__`?**
...
- **Should `RecoveryEngine` be split into smaller, more focused modules?**
