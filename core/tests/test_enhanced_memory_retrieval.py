...
def test_tokenize_text_and_tf_idf():
    ...
    score = compute_tf_idf_score(["memory", "bm25"], ["refactored", "memory", "module", "with", "bm25"])
    assert score > 0.0
...
def test_hybrid_memory_retriever_channel_filtering():
    ...
    mem1 = MemoryObject(
        ...
        channel_id="telegram",
        vector_tokens=tokenize_text("Use Telegram bot gateway with asyncio for multi-channel messaging"),
    )
    mem2 = MemoryObject(
        kind="decision",
        summary="Use Slack Bolt SDK for enterprise workspace integration",
        channel_id="slack",
        vector_tokens=tokenize_text("Use Slack Bolt SDK for enterprise workspace integration"),
    )
    mem3 = MemoryObject(
        kind="fact",
        summary="CLI default persistent memory file is .context-memory.json",
        channel_id="default",
        vector_tokens=tokenize_text("CLI default persistent memory file is .context-memory.json"),
    )

    memories = [mem1, mem2, mem3]

    # Query telegram channel
    results_tg = retriever.retrieve("Telegram gateway messaging", memories, channel_id="telegram", top_k=5)
    ...
    # Query slack channel
    results_slack = retriever.retrieve("Slack Bolt workspace", memories, channel_id="slack", top_k=5)
...
def test_project_memory_add_and_search():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mo1 = MemoryObject(
            ...
            channel_id="discord",
            vector_tokens=tokenize_text("Decoupled core memory from CLI frontend using hybrid retrieval"),
        ...
        # Search memory
        search_res = pm.search_memory("hybrid retrieval core memory", channel_id="discord")
...
def test_tiered_memory_l3_scratchpad_surfacing():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        mo = MemoryObject(
            ...
            channel_id="default",
            vector_tokens=tokenize_text("Optimized KV cache with 4-bit TurboQuant on 8GB Mac"),
        )
        ...
        assert ("Facts & Past Context" in scratchpad or "Relevant L3 Context" in scratchpad)
...
def test_l0_scratchpad_priority_weighted_order():
    ...
    tm.state.errors_seen = ["Err1", "Err2", "Err3"]  # last 2 unique
    ...
    tm.state.decisions = ["Dec1", "Dec2", "Dec3", "Dec4"]  # max 3 recent
    ...
    assert "Err1" not in scratchpad  # capped at last 2 unique
    assert "Dec1" not in scratchpad  # capped at last 3 recent
