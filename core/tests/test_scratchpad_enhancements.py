...
def test_save_memory_tool(tmp_path):
    ...
    res = registry.execute("SAVE_MEMORY", {"entry": "Use Redis for session caching", "category": "arch_decision"}, project_root=str(tmp_path))
...
def test_update_task_graph_tool(tmp_path):
    ...
    harness.initialize_goal("goal_1", "Test Goal", "Test Description", tasks=[
        {"id": "task_1", "description": "Initial task", "target_files": ["src/main.py"]}
    ])

    # Test adding a subtask via tool call
    res = registry.execute("UPDATE_TASK_GRAPH", {
        "action": "add_subtask",
        "task_id": "task_2",
    ...
    res_skip = registry.execute("UPDATE_TASK_GRAPH", {
        "action": "skip_task",
        "task_id": "task_1",
    }, project_root=str(tmp_path))
...
def test_autonomous_harness_ast_symbol_handoff(tmp_path):
    ...
    target_file.write_text("def authenticate_user(): pass\nclass SessionManager: pass\n", encoding="utf-8")

    memory = TieredMemory(MemoryConfig(max_tokens=4096))
    harness = AutonomousHarness(memory=memory, project_root=str(tmp_path))
    harness.initialize_goal("goal_ast", "AST Goal", "Test AST summary generation", tasks=[
        {"id": "task_ast", "description": "Create service module", "target_files": ["src/service.py"]}
    ])
    ...
    assert "authenticate_user" in task.outputs_summary or "SessionManager" in task.outputs_summary
