...
class RLMEngine:
    ...
    def solve(self, task: str, depth: int = 0) -> SolveResult:
        ...
        if TieredMemory and MemoryConfig:
            ...
            memory = TieredMemory(
                config=MemoryConfig.auto_tune(
                    max_tokens=CTX_SIZE,
                    metadata_overhead=estimate_metadata_overhead(
                        system_content=SYSTEM_PROMPT, ctx_size=CTX_SIZE
                    ),
                )
            )
        ...
        REPEAT_LIMIT = (
            2  # how many identical actions in a row before we force an answer
        )
        ...
        for iteration in range(MAX_ITERATIONS_PER_LEVEL):
            ...
            # --- Repeat detection
            signature = (
                (action, content.strip()) if action in ("code", "sub_query") else None
            )
            ...
            if action == "final_answer":
                ...
                if depth >= self.max_depth:
                    ...
                    if use_memory:
                        ...
                        memory.add_user_message(
                            build_step_message("sub_query_result", sub_result.answer)
                        )
                    else:
                        messages.append({"role": "assistant", "content": response})
                        messages.append(
                            {
                                ...
                                "content": build_step_message(
                                    "sub_query_result", sub_result.answer
                                ),
                            }
                        )
                ...
                if use_memory:
                    memory.add_assistant_message(response)
                    memory.add_user_message(
                        "Please continue with an action. Use <CODE>, <SUB_QUERY>, or <FINAL_ANSWER>."
                    )
                else:
                    messages.append({"role": "assistant", "content": response})
                    messages.append(
                        {
                            "role": "user",
                            "content": "Please continue with an action. Use <CODE>, <SUB_QUERY>, or <FINAL_ANSWER>.",
                        }
                    )
    ...
    def _parse_response(self, response: str) -> tuple[str, str, str]:
        ...
        is_planning_cot = (
            ...
            or bool(
                re.search(
                    r"\b(?:LIST_DIR|READ_FILE|EDIT_FILE|WRITE_FILE|GREP|SEARCH_AST|EXECUTE|RUN_COMMAND)\b",
                    cleaned_body,
                )
            )
            or bool(
                re.match(
                    r"^(?:1[\.\s]|step\s*1|first,|I\s+will\s+start|I\s+need\s+to\s+first)",
                    cleaned_body,
                    re.IGNORECASE,
                )
            )
