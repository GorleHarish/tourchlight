...
def get_project_structure(project_root: str = "."):
    ...
    try:
        results = conn.execute("MATCH (f:File)-[:File_HAS_CLASS]->(c:ClassDef) RETURN f.filepath, c.name")
        structure_map = {}
        ...
        results2 = conn.execute("MATCH (f:File)-[:File_HAS_FUNCTION]->(fn:FunctionDef) RETURN f.filepath, fn.name")
...
def get_class_signature(class_name, project_root: str = "."):
    ...
    if conn is None:
        return f"Error opening Kuzu graph database at {db_path}."
    try:
        results = conn.execute("MATCH (c:ClassDef {name: $name}) RETURN c.docstring LIMIT 1", parameters={"name": class_name})
        if not results.has_next():
            return f"Class {class_name} not found."
            
        docstring = results.get_next()[0]
        methods_res = conn.execute("MATCH (c:ClassDef {name: $name})-[:Class_HAS_FUNCTION]->(fn:FunctionDef) RETURN fn.name, fn.args", parameters={"name": class_name})
...
def get_function_ast(func_name, project_root: str = "."):
    ...
    if conn is None:
        return f"Error opening Kuzu graph database at {db_path}."
    try:
        res = conn.execute("MATCH (fn:FunctionDef {name: $name}) RETURN fn.ast_dump LIMIT 1", parameters={"name": func_name})
        if res.has_next():
...
def get_function_source(func_name, project_root: str = "."):
    ...
    if conn is None:
        return f"Error opening Kuzu graph database at {db_path}."
    try:
        res = conn.execute("MATCH (fn:FunctionDef {name: $name}) RETURN fn.source_code LIMIT 1", parameters={"name": func_name})
        if res.has_next():
            return res.get_next()[0]
        return f"Function {func_name} not found."
    except Exception as e:
...
def semantic_search(query_string, top_k=3, project_root: str = "."):
    ...
    try:
        ...
        while res_class.has_next():
            ...
            class_results.append(f"[Class] ID: {row[0]}, Name: {row[1]}, Sim: {row[2]:.3f}")
        ...
        while res_func.has_next():
            ...
            func_results.append(f"[Function] ID: {row[0]}, Name: {row[1]}, Sim: {row[2]:.3f}")
...
def get_local_subgraph(node_id, project_root: str = "."):
    ...
    try:
        ...
        if node_id.endswith(".py"):
            res = conn.execute("MATCH (f:File)-[*1..2]-(m) WHERE f.filepath = $id RETURN label(m), coalesce(m.id, m.filepath) LIMIT 20", {"id": node_id})
            while res.has_next():
                r = res.get_next()
                neighbors.append(f"[{r[0]}] {r[1]}")
        else:
            res1 = conn.execute("MATCH (n:ClassDef)-[*1..2]-(m) WHERE n.id = $id RETURN label(m), coalesce(m.id, m.filepath) LIMIT 20", {"id": node_id})
            while res1.has_next():
                r = res1.get_next()
                neighbors.append(f"[{r[0]}] {r[1]}")
                
            res2 = conn.execute("MATCH (n:FunctionDef)-[*1..2]-(m) WHERE n.id = $id RETURN label(m), coalesce(m.id, m.filepath) LIMIT 20", {"id": node_id})
            while res2.has_next():
                r = res2.get_next()
                neighbors.append(f"[{r[0]}] {r[1]}")
...
class REPLSandbox:
    def __init__(self, llm_query_fn: Optional[Callable] = None, project_root: Optional[str] = None):
    ...
    def set_project_root(self, project_root: str) -> None:
        ...
        self._namespace["get_project_structure"] = lambda: get_project_structure(self._project_root)
        self._namespace["get_class_signature"] = lambda name: get_class_signature(name, self._project_root)
        self._namespace["get_function_ast"] = lambda name: get_function_ast(name, self._project_root)
        self._namespace["get_function_source"] = lambda name: get_function_source(name, self._project_root)
        self._namespace["semantic_search"] = lambda query, top_k=3: semantic_search(query, top_k, self._project_root)
        self._namespace["get_local_subgraph"] = lambda node_id: get_local_subgraph(node_id, self._project_root)
    ...
    def _build_namespace(self) -> dict:
        ...
        namespace["get_project_structure"] = lambda: get_project_structure(self._project_root)
        namespace["get_class_signature"] = lambda name: get_class_signature(name, self._project_root)
        namespace["get_function_ast"] = lambda name: get_function_ast(name, self._project_root)
        namespace["get_function_source"] = lambda name: get_function_source(name, self._project_root)
        namespace["semantic_search"] = lambda query, top_k=3: semantic_search(query, top_k, self._project_root)
        namespace["get_local_subgraph"] = lambda node_id: get_local_subgraph(node_id, self._project_root)
    ...
    def _safe_import(self, name, *args, **kwargs):
        ...
        raise ImportError(f"Module '{name}' is not allowed. Available: {', '.join(ALLOWED_MODULES)}")
    ...
    def get_variables(self) -> dict:
        ...
        return {k: repr(v) for k, v in self._namespace.items() if k not in skip and not k.startswith("_")}
